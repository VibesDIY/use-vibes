...
export function kanban(doc, oldDoc, user, ctx) {
  ...
  switch (doc.type) {
    ...
    case "task": {
      if (oldDoc && doc.boardId !== oldDoc.boardId && !user.isOwner) throw { forbidden: "boardId is immutable" };
      if (oldDoc) {
        if (doc.authorHandle !== oldDoc.authorHandle && !user.isOwner) throw { forbidden: "authorHandle is immutable" };
      ...
      if (boardId.startsWith("default-")) {
        return { channels: [chan], grant: { users: { [boardId.slice("default-".length)]: [chan, chan + "/admin"] } } };
      }
