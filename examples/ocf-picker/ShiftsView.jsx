...
export default function ShiftsView({
  ...
  return (
    <div>
      <h2 className={`text-2xl font-black font-serif mb-1.5 ${c.bodyText}`}>Manage Your Extras</h2>
      {canWrite && (
            ...
            <div key={shift._id} className={c.shiftCard}>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  {canWrite && (
                    <label
                      className={`mt-0.5 inline-flex items-center gap-0.5 text-sm font-bold text-white`}
                    >
                      <input
                        type="checkbox"
      ...
      {shifts.length === 0 && (
        <div className="text-center py-3">
          ...
          <p className={c.bodyText}>
            Use the form above to add shifts, meals, breaks, or anything else
          </p>
        </div>
      )}
    </div>
