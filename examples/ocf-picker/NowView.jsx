  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5 flex-wrap gap-[3px]">
        <h2 className={`text-2xl font-black font-serif ${c.bodyText}`}>Right Now</h2>
        <p className={`text-sm font-bold ${c.bodyText}`}>
      ...
      {nowSets.length === 0 ? (
        <div className="mb-1.5 p-2 bg-[#efe4cf] dark:bg-[#1e1a12] rounded-2xl m-0.5 ">
      ...
      <h3 className={`text-xl font-black font-serif mb-[3px] ${c.bodyText}`}>Up Next</h3>
      {nextSets.length === 0 ? (
        <div className="p-2 bg-[#efe4cf] dark:bg-[#1e1a12] rounded-2xl m-0.5 ">
          <p className={`font-bold ${c.bodyText}`}>No more events scheduled.</p>
        </div>
      ) : (
