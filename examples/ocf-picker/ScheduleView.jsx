...
function GapStrip({ startMs, endMs, allDayEvents, fmtTime }) {
  ...
  return (
    <div className="rounded-lg m-0.5  px-[7px] py-[5px] bg-white/40 dark:bg-white/10 flex items-center gap-0.5">
      <span className="text-xs font-bold text-[#3a2f28]/60 dark:text-[#f0e9df]/60">
        {startStr}–{endStr} · {count} event{count !== 1 ? 's' : ''}
      </span>
    </div>
  );
}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-0.5 flex-wrap mb-[1px]">
                          ...
                          {isEvent && onToggleFavorite && (
                            <button
                              ...
                              className={`p-[1px] rounded-lg m-0.5  text-xs font-bold px-0.5 ${myFavIds && myFavIds.has(item.data.eventId) ? 'bg-[#d95931] text-white' : 'bg-[#efe4cf] dark:bg-[#1e1a12] text-[#3a2f28] dark:text-[#f0e9df]'}`}
                            >
                        ...
                        {isEvent &&
                          (canWrite && saveNote ? (
                            ...
                            <div
                              className={`mt-0.5 p-1.5 bg-[#efe4cf] dark:bg-[#1e1a12] rounded-lg m-0.5 `}
                            >
                              <p className={`text-sm font-bold ${c.bodyText}`}>
