...
describe('migrateRollingDoc (anon local → cloud on sign-in)', () => {
  it('re-keys a favorite onto the signed-in handle', () => {
    expect(
      migrateRollingDoc({ type: 'favorite', rideId: '42', userId: 'anonymous' }, 'alice')
    ).toEqual({
      type: 'favorite',
      rideId: '42',
      userId: 'alice',
  ...
  it('re-keys a note onto the signed-in handle', () => {
    expect(
      migrateRollingDoc({ type: 'note', rideId: '7', notes: 'bring lights' }, 'alice')
    ).toEqual({
      type: 'note',
      rideId: '7',
      notes: 'bring lights',
...
describe('access.js (channel routing + ownership + guards)', () => {
  it('favorites are public-read, owner-write', () => {
    const { ok } = run({ type: 'favorite', userId: 'alice', rideId: '1' }, null, {
      userHandle: 'alice',
    });
  ...
  it('rejects a favorite written for someone else', () => {
    expect(
      run({ type: 'favorite', userId: 'bob', rideId: '1' }, null, { userHandle: 'alice' })
    ).toEqual({
      forbidden: 'not owner',
    });
  });
  ...
  it("notes are private to their owner's channel", () => {
    const { ok } = run({ type: 'note', userId: 'alice', rideId: '1', notes: 'x' }, null, {
      userHandle: 'alice',
    });
  ...
  it('a friend edge is visible to both endpoints', () => {
    const { ok } = run({ type: 'friend', userId: 'alice', friendSlug: 'bob' }, null, {
      userHandle: 'alice',
    });
  ...
  it('uses oldDoc as a fallback for delete tombstones', () => {
    ...
    const { ok } = run(
      { _id: 'favorite-alice-1', _deleted: true },
