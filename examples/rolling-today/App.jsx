...
export default function App() {
  ...
  const notes = useMemo(
    () => Object.fromEntries(notesDocs.map((n) => [n.rideId, n.notes])),
    [notesDocs]
  );
  ...
  const favsByRide = useMemo(
    () => visibleFavsByRide(favorites, visibleSlugs),
  ...
  const loadDate = useCallback(async (target, opts = {}) => {
    ...
    try {
      ...
      setEvents(found);
      if (landed !== target)
        setSkipNotice(`No rides on ${prettyDate(target)} — jumped to ${prettyDate(landed)}`);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    database
      .put({
        _id: `friend-${userId}-${linkedFriend}`,
        type: 'friend',
        userId,
  ...
  // Live calendar subscription (backend.js GET /_api/faves.ics?t=<token>).
  // The token is a per-user random CAPABILITY, opt-in: nothing exists until
  // the user taps "Create calendar link". Unlike a handle-keyed URL it is
  // unguessable, and revocable (delete the caltoken doc). New stars still
  // reach subscribers automatically, and sharing the link lets a friend
  // follow your rides. Signed-in only — anonymous stars never reach the
  // cloud. Legacy favorites carry userSlug instead of userId, so count both.
  const myFavCount = useMemo(
    () => favorites.filter((f) => (f.userId ?? f.userSlug ?? 'anonymous') === userId).length,
    [favorites, userId]
  );
  const { docs: calTokens } = useLiveQuery(byTypeUser, { key: ['caltoken', userId] });
  const calToken = calTokens[0]?.token || null;
  // LOCAL MINTING: the token is generated client-side and put the moment the
  // Favorites view opens with subscribable content — the optimistic write
  // makes it visible to the live query (and thus the button URL) instantly,
  // and by the time a subscriber's calendar validates the URL, the backend's
  // ≤1m tick has usually caught up (until then the endpoint serves a valid
  // placeholder calendar, so nothing fails). Opt-in still holds: users who
  // never open this view never get a token or an ics aggregate.
  useEffect(() => {
    if (view !== 'favorites' || !signedIn || myFavCount === 0 || calToken) return;
    ...
    database
      ...
      .catch(() => {});
  }, [view, signedIn, myFavCount, calToken, userId, database]);
  const icsSubPath =
    signedIn && myFavCount > 0 && calToken
          ...
          <nav className={c.nav}>
            ...
            >
              ...
              <span className="hidden sm:inline">
                Friends{friendSlugs.size > 0 ? ` (${friendSlugs.size})` : ''}
              </span>
            </button>
          </nav>
        </header>
        ...
        <div className="fixed bottom-[10px] left-3 right-3 z-40 pointer-events-none">
          ...
          <div className={c.signinCallout} style={{ marginBottom: 0 }}>
            <span className="min-w-0 flex-1">
              Sign in via the Vibes DIY logo to sync your rides and follow friends
            </span>
