...
export const fetchFeedWindow = async () => {
  ...
  const res = await globalThis.fetch(`${FEED_URL}?startdate=${start}&enddate=${end}`, {
  ...
  for (const ev of data.events) {
    const key =
      ev?.caldaily_id != null ? String(ev.caldaily_id) : ev?.id != null ? String(ev.id) : null;
    if (key !== null) byId.set(key, ev);
  }
  return byId;
};

const rideDescription = (r) =>
  [r.organizer ? `Led by ${r.organizer}` : '', r.timedetails || ''].filter(Boolean).join('\n');
...
const handleSubscription = async (url) => {
  ...
  return new Response(
    buildFavesCalendar(items, { calName: `@${handle ?? displayName ?? 'my'} — Rolling Today` }),
    {
      status: 200,
      headers: {
