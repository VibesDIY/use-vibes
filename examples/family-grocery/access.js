  ...
  switch (type) {
    case "group": {
      ...
      if (oldDoc) {
        ...
        if (!doc._deleted && oldDoc.creatorHandle !== undefined && doc.creatorHandle !== oldDoc.creatorHandle && !user.isOwner) {
    ...
    case "item": {
      ...
      if (oldDoc && !doc._deleted && !user.isOwner) {
        if (oldDoc.groupId !== undefined && doc.groupId !== oldDoc.groupId) throw { forbidden: "groupId is immutable" };
      ...
      if (groupId.startsWith("default-")) {
        return { channels: [chan], grant: { users: { [groupId.slice("default-".length)]: [chan, chan + "/admin"] } } };
      }
    ...
    case "member": {
      ...
      if (groupId.startsWith("default-")) {
        return {
          channels: [chan],
          grant: { users: { [safeId(memberHandle)]: [chan], [groupId.slice("default-".length)]: [chan, chan + "/admin"] } },
        };
      }
