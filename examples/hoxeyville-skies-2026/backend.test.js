...
const items = (...overrides) =>
  overrides.length > 0
    ...
    : [
        {
          id: 'event-fri-hoxey-grahame-lesh-and-friends',
          title: 'Grahame Lesh & Friends',
...
describe('toIcsUtc — festival-local strings become ICS UTC stamps', () => {
  it('converts a naive festival-local time (EDT, UTC-4) to Z time', () => {
    expect(toIcsUtc('2026-07-31T13:00:00')).toBe('20260731T170000Z');
  });
  it('handles DST correctly — a winter time is EST (UTC-5)', () => {
    expect(toIcsUtc('2026-12-01T12:00:00')).toBe('20261201T170000Z');
  });
  ...
  it("accepts the feed's space-separated form", () => {
    expect(toIcsUtc('2026-07-31 13:00:00')).toBe('20260731T170000Z');
  });
...
describe('parseFavesItems — strict payload validation', () => {
  it('accepts a minimal valid payload and normalizes times to UTC', () => {
    ...
    expect(r.items[0]).toMatchObject({
      title: 'Grahame Lesh & Friends',
      start: '20260731T170000Z',
      end: '20260731T180000Z',
    });
  });
      ...
      items: [
        {
          ...
          location: 'Hoxey Stage',
          url: 'javascript:alert(1)',
        },
      ],
    });
    expect(r.ok).toBe(true);
    expect(r.items[0].location).toBe('Hoxey Stage');
  ...
  it('normalizes an overnight extra (same-day 22:00 → 01:00) to end the next day', () => {
    ...
    expect(r.ok).toBe(true);
    // 22:00 EDT Jul 30 → 02:00Z Jul 31; 01:00 EDT bumped to Jul 31 → 05:00Z Jul 31.
    expect(r.items[0].start).toBe('20260731T020000Z');
    expect(r.items[0].end).toBe('20260731T050000Z');
  });
...
describe('buildFavesCalendar — the ICS document', () => {
  ...
  it('has the calendar envelope and metadata', () => {
    ...
    expect(ics).toContain('X-WR-CALNAME:My Hoxeyville Skies Picks\r\n');
    expect(ics).toContain('X-WR-TIMEZONE:America/Detroit\r\n');
    expect(ics.trimEnd().endsWith('END:VCALENDAR')).toBe(true);
  });
  it('emits events sorted by start time regardless of input order', () => {
    // 2026-08-01T20:00 EDT crosses midnight UTC → 20260802T000000Z, and it must come second.
    expect(ics.indexOf('20260731T170000Z')).toBeLessThan(ics.indexOf('20260802T000000Z'));
    expect(ics.indexOf('20260802T000000Z')).toBeGreaterThan(-1);
  });
  it('stamps stable UIDs from the item id', () => {
    expect(ics).toContain('UID:event-1@hoxeyville-skies-picker.vibes.diy');
    expect(ics).toContain('UID:event-2@hoxeyville-skies-picker.vibes.diy');
  });
  it('escapes SUMMARY text and carries LOCATION/URL', () => {
    expect(ics).toContain('SUMMARY:Früh\\; und\\, spät');
    expect(ics).toContain('LOCATION:Hoxey Stage');
...
describe('scheduleItemsFor — the embedded-snapshot join', () => {
  it('projects known ids into calendar items straight from the snapshot', () => {
    expect(scheduleItemsFor(['fri-hoxey-grahame-lesh-and-friends'])).toEqual([
      {
        id: 'event-fri-hoxey-grahame-lesh-and-friends',
        title: 'Grahame Lesh & Friends',
        start: '2026-08-07T21:00:00',
        end: '2026-08-08T00:00:00', // the Friday headliner runs to midnight
        location: 'Hoxey Stage',
        url: 'https://www.hoxeyvilleskies.com/schedule',
      },
    ]);
  });
  it('drops a set the festival published without an end time', () => {
    // The two "12:00 Acoustic" sets carry a start and a null end, so they
    // cannot become calendar events — they must vanish from the join's output
    // rather than emit a VEVENT with no DTEND.
    expect(scheduleItemsFor(['fri-gopherwood-smokin-dobroleles-acoustic'])[0].end).toBe('');
  });
...
const DB_DOCS = [
  {
    _id: 'favorite-Alice-fri-hoxey-grahame-lesh-and-friends',
    type: 'favorite',
    userId: 'Alice',
    eventId: 'fri-hoxey-grahame-lesh-and-friends',
  },
  ...
  {
    _id: 'favorite-bob-sat-hoxey-ekoostik-hookah',
    type: 'favorite',
    userId: 'bob',
    eventId: 'sat-hoxey-ekoostik-hookah',
  },
  {
    _id: 'shift-1',
    type: 'shift',
  ...
  {
    _id: 'note-alice-fri-hoxey-grahame-lesh-and-friends',
    type: 'note',
    userId: 'alice',
    eventId: 'fri-hoxey-grahame-lesh-and-friends',
    notes: 'PRIVATE NOTE',
...
describe('fetch handler — GET /faves.ics?t=<token> (subscription lane)', () => {
  ...
  it("serves a user's CURRENT faves: db-aggregated ids joined against the snapshot", async () => {
    ...
    const body = await res.text();
    expect(body).toContain('SUMMARY:Grahame Lesh & Friends'); // alice faved fri-hoxey-grahame-lesh-and-friends (case-folded handle)
    expect(body).not.toContain('ekoostik hookah'); // sat-hoxey-ekoostik-hookah is bob's
    ...
    expect(body).toContain('LOCATION:Hoxey Stage');
    // Unfold first: this festival's slug-shaped ids push the UID line past the
    // 75-octet limit, so it arrives split across a CRLF + space continuation.
    expect(body.replace(/\r\n /g, '')).toContain(
  ...
  it('drops a favorite the snapshot no longer lists instead of failing the feed', async () => {
    // alice also faves `act-gone`; her calendar is anchor + fri-hoxey-grahame-lesh-and-friends + her shift.
    await tick();
  ...
  it('serves the whole feed without any egress — the snapshot replaced the fetch', async () => {
    ...
    expect(await res.text()).toContain('SUMMARY:Grahame Lesh & Friends');
  });
  ...
  it('derives legacy shift times from day + startTime/endTime, using the snapshot dates', async () => {
    ...
    expect(body).toContain('DTSTART:20260807T140000Z'); // Friday (2026-08-07) 10:00 EDT
  });
...
describe('fetch handler — POST /faves.ics', () => {
  ...
  it('returns a text/calendar attachment for a valid payload', async () => {
    ...
    expect(res.headers.get('content-disposition')).toBe(
      'attachment; filename="hoxeyville-skies-faves.ics"'
    );
    expect(res.headers.get('cache-control')).toBe('no-store');
    const body = await res.text();
    expect(body).toContain('BEGIN:VEVENT');
    expect(body).toContain('SUMMARY:Grahame Lesh & Friends');
    expect(body).toContain('DTSTART:20260731T170000Z'); // festival-local converted to UTC
  });
