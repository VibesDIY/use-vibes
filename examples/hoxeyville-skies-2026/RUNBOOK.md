# Hoxeyville Skies 2026 — Picker Runbook
...
a calendar. This directory is an **instantiation** of `examples/festival-picker-template`
for Hoxeyville Skies Music Festival — Wellston, MI, August 7–8, 2026.

- **Live:** <https://vibes.diy/vibe/festival/hoxeyville-skies-2026>
- **Source of truth:** the official 2026 schedule graphic linked from
  <https://www.hoxeyvilleskies.com/schedule> (lineup roster at
  <https://www.hoxeyvilleskies.com/lineup>). 20 sets across the Hoxey Stage and the
  Gopherwood Stage; every entry in `festival-config.js` carries that provenance URL.
- **Tier `full`:** per-set times and stages are published. The two midnight
  "12:00 Acoustic" sets are published with a start and no end, so they carry
  `end: null` — nothing is invented to fill the gap, and they simply never become
  calendar events.

The rest of this file is the template's own runbook, kept as-is so next year's
re-snapshot follows the same steps.
