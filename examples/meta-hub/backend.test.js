...
describe("liPostBody — article-share request body", () => {
  ...
  it("falls back to slug for the title (LinkedIn does not scrape the URL)", () => {
    const { post } = liPostBody(author, { slug: "my-slug", caption: "https://good.vibes.diy/blog/my-slug" });
  ...
  it("errors without a link anywhere", () => {
    expect(liPostBody(author, { slug: "s", caption: "no link here" }).error).toMatch(/needs a link/);
  });
  it("errors when escaped commentary exceeds the cap", () => {
    const { error } = liPostBody(author, { slug: "s", caption: `https://x.example ${"(".repeat(LI_MAX_TEXT / 2)}` });
...
describe("liFetch — LinkedIn REST client", () => {
  ...
  it("sends Bearer + version headers and returns the x-restli-id of an empty 201", async () => {
    const f = stub(async () => new Response(null, { status: 201, headers: { "x-restli-id": "urn:li:share:42" } }));
  ...
  it("surfaces LinkedIn's error shape with the status a dead token needs", async () => {
    stub(
      async () =>
        new Response(JSON.stringify({ serviceErrorCode: 65600, message: "Invalid access token", status: 401 }), { status: 401 })
    );
  ...
  it("recognizes the egress proxy's denial marker", async () => {
    stub(
      async () => new Response(JSON.stringify({ vibesEgressDenied: true, gate: "cors", host: "api.linkedin.com" }), { status: 403 })
    );
...
describe("bskyParseCredential — identifier:app-password paste", () => {
  it("splits on the LAST colon (app passwords are xxxx-xxxx-xxxx-xxxx, no colons)", () => {
    ...
    expect(bskyParseCredential("me@example.com: pw ")).toEqual({ identifier: "me@example.com", password: "pw" });
  });
...
describe("bskyLinkFacets — UTF-8 byte-offset link facets", () => {
  it("marks a URL with byte offsets (links are NOT auto-detected by Bluesky)", () => {
    ...
    expect(bskyLinkFacets(text)).toEqual([
      { index: { byteStart: 5, byteEnd: 24 }, features: [{ $type: "app.bsky.richtext.facet#link", uri: "https://x.example/p" }] },
    ]);
  });
...
describe("bskyPostRecord — post record with link facet + website card", () => {
  it("builds the record: text, facets, external embed with title/description", () => {
    const { record, error } = bskyPostRecord(
      { slug: "s", caption: "hook https://good.vibes.diy/blog/s?src=bsky", title: "My Post", description: "d" },
    ...
    expect(record.embed).toEqual({
      ...
      external: { uri: "https://good.vibes.diy/blog/s?src=bsky", title: "My Post", description: "d" },
    });
  });
  it("prefers an explicit link, falls back to slug for the title and empty description", () => {
    const { record } = bskyPostRecord(
      { slug: "my-slug", caption: "see https://other.example", link: "https://good.vibes.diy/blog/my-slug" },
      "t"
    );
  ...
  it("errors past 300 chars (code points) and without a link", () => {
    expect(bskyPostRecord({ slug: "s", caption: `https://x.example ${"a".repeat(BSKY_MAX_TEXT)}` }, "t").error).toMatch(/max 300/);
...
const LI_LANE_DENIED = ["api.linkedin.com/", () => json({ vibesEgressDenied: true, gate: "cors", host: "api.linkedin.com" }, 403)];
...
describe("scheduled — shared-path invariants (ig/threads/fbpage parity)", () => {
  ...
  it("rotates a stale ig token but never fbpage or linkedin", async () => {
    const calls = routedFetch([
      META_PROBE_LIVE,
      LI_LANE_LIVE,
      ["graph.instagram.com/refresh_access_token", () => json({ access_token: "IGTOK2", expires_in: 5184000 })],
    ]);
    const ctx = mockCtx({
      vault: [
  ...
  it("Meta code 190 during publish flags the vault and HOLDS the request (no terminal error)", async () => {
    routedFetch([
      META_PROBE_LIVE,
      LI_LANE_LIVE,
      ["graph.instagram.com/v23.0/123/media", () => json({ error: { message: "Session has expired", code: 190 } }, 400)],
    ]);
    const ctx = mockCtx({
      vault: [igToken()],
      requests: [
  ...
  it("lane gating is per-platform: LinkedIn lane down holds only linkedin requests", async () => {
    routedFetch([META_PROBE_LIVE, LI_LANE_DENIED, ["graph.instagram.com/v23.0/123/media", () => json({ id: "container-1" })]]);
    const ctx = mockCtx({
      vault: [
        igToken(),
        {
  ...
  it("lane gating parity the other way: Meta egress down holds ig while linkedin publishes", async () => {
    routedFetch([
      ...
      LI_LANE_LIVE,
      ["api.linkedin.com/rest/posts", () => new Response(null, { status: 201, headers: { "x-restli-id": "urn:li:share:9" } })],
    ]);
    const ctx = mockCtx({
      vault: [
        igToken(),
  ...
  it("bsky publishes via refreshSession, persists rotated JWTs AND the current handle — no createSession on a live session", async () => {
    const calls = routedFetch([
      ...
      [
        ...
        () => json({ accessJwt: "A2", refreshJwt: "R2", did: "did:plc:abc", handle: "renamed.example" }),
      ],
      ["com.atproto.repo.createRecord", () => json({ uri: "at://did:plc:abc/app.bsky.feed.post/3k2xyz", cid: "bafy..." })],
    ]);
    const ctx = mockCtx({
      vault: [
        {
    ...
    const calls = routedFetch([
      ...
      [
        ...
        (url, init) => {
          ...
          if ((init.headers || {}).authorization !== "Bearer R1") return json({ error: "ExpiredToken" }, 400);
          return json({ accessJwt: "A2", refreshJwt: "R2", did: "did:plc:abc", handle: "vibes.diy" });
        },
      ],
      ["com.atproto.repo.createRecord", () => json({ uri: "at://did:plc:abc/app.bsky.feed.post/3kbatch", cid: "bafy..." })],
    ]);
    const ctx = mockCtx({
      vault: [
        {
  ...
  it("bsky falls back to createSession from the app password when the refresh JWT is dead", async () => {
    routedFetch([
      META_PROBE_LIVE,
      LI_LANE_LIVE,
      ["com.atproto.server.refreshSession", () => json({ error: "ExpiredToken", message: "Token has expired" }, 400)],
      [
        "com.atproto.server.createSession",
        () => json({ accessJwt: "A9", refreshJwt: "R9", did: "did:plc:abc", handle: "vibes.diy" }),
      ],
      ["com.atproto.repo.createRecord", () => json({ uri: "at://did:plc:abc/app.bsky.feed.post/3k9", cid: "bafy..." })],
    ]);
    const ctx = mockCtx({
      vault: [
        {
