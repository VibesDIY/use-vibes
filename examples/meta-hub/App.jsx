...
function StatusChip({ status }) {
  ...
  return (
    <span className={`text-[11px] px-[8px] py-[2px] rounded-full border ${tone}`}>{status}</span>
  );
}
...
export default function App() {
  ...
  const requests = reqQ('kind', { key: 'publish-request' }).docs.sort((a, b) =>
    (b.createdAt || '').localeCompare(a.createdAt || '')
  );
  const oplog = logQ('kind', { key: 'oplog' }).docs.sort((a, b) =>
      ...
      <div className="max-w-[760px] mx-auto">
        <header className="mb-[20px]">
          ...
          <p className="text-[12px] text-stone-400">
            Token vault · rotation · IG/Threads/FB/LinkedIn/Bluesky publishing — only you can see
            this.
          </p>
          ...
          <p className="text-[11px] text-stone-500">
            ...
            <span className={probeLi?.status === 'live' ? 'text-emerald-400' : 'text-red-400'}>
              {probeLi
                ? `${probeLi.status} — ${probeLi.detail}`
                : 'no probe yet (first tick pending)'}
            </span>
          </p>
        </header>
        ...
        <section className="mb-[24px] bg-[#1b2329] border border-stone-700 rounded-xl p-[16px]">
          ...
          {tokens.length === 0 && (
            <p className="text-[12px] text-stone-500 mb-[10px]">
              Vault is empty — paste a token below.
            </p>
          )}
          {justSaved && (
          ...
          <div className="flex gap-[8px] mt-[8px]">
            ...
            <input
              ...
              placeholder={
                platform === 'bsky' ? 'paste identifier:app-password' : 'paste long-lived token'
              }
        ...
        <section className="mb-[24px] bg-[#1b2329] border border-stone-700 rounded-xl p-[16px]">
          ...
          {requests.map((r) => (
            <div
              key={r._id}
              className="text-[12px] border-b border-stone-800 py-[8px] last:border-0"
            >
              <div className="flex items-center gap-[8px]">
              ...
              {r.permalink && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-[#FEDD00] underline"
                >
                  {r.permalink}
                </a>
              )}
