...
export async function liFetch(path, { method = "GET", body, token } = {}) {
  ...
  if (data && data.vibesEgressDenied) return { ok: false, egressDenied: data.gate || true };
  if (!res.ok) {
    return { ok: false, status: res.status, code: data.serviceErrorCode, message: data.message || `HTTP ${res.status}` };
  }
...
export function liPostBody(author, req) {
  ...
  if (!link) return { error: "linkedin article share needs a link (a `link` field or a URL in the caption)" };
  const commentary = liEscape(text);
  if (commentary.length > LI_MAX_TEXT) {
    return { error: `linkedin text is ${commentary.length} chars after escaping (max ${LI_MAX_TEXT})` };
  }
  return {
    post: {
      author,
  ...
  return {
    record: {
      ...
      embed: {
        ...
        external: { uri: link, title: req.title || req.slug || link, description: req.description || "" },
      },
    },
  };
}
  ...
  await ctx.db.put(
    {
      ...
      op: "egress-probe",
      status,
      detail: r.egressDenied ? `gate: ${r.egressDenied}` : r.transport || r.message || "graph reachable",
  ...
  await ctx.db.put(
    {
      ...
      op: "egress-probe",
      status,
      detail: r.egressDenied ? `gate: ${r.egressDenied}` : r.transport || r.message || "linkedin reachable",
      at: new Date().toISOString(),
    },
    { db: "oplog" }
  );
...
async function enrichToken(ctx, t) {
  if (t.platform === "bsky") {
    ...
    if (s.accessJwt) {
      ...
      await ctx.db.put({ ...s.t, expiresAt: null, needsReauth: false, lastError: null }, { db: "vault" });
      ...
      await log(ctx, { op: "token-verify-failed", platform: t.platform, error: s.r.message || s.r.egressDenied });
    }
    return;
  }
  if (t.platform === "linkedin") {
    ...
    if (r.ok && r.data.sub) {
      await ctx.db.put(
        { ...t, igUserId: `urn:li:person:${r.data.sub}`, username: r.data.name || null, needsReauth: false, lastError: null },
        { db: "vault" }
      );
      await log(ctx, { op: "token-verified", platform: t.platform, username: r.data.name || r.data.sub });
    } else {
      await ctx.db.put({ ...t, needsReauth: liTokenDead(r), lastError: r.message || JSON.stringify(r) }, { db: "vault" });
      await log(ctx, { op: "token-verify-failed", platform: t.platform, error: r.message || r.egressDenied });
    }
    return;
  }
  ...
  if (r.ok) {
    let resolved = { igUserId: r.data.id, username: r.data.username || r.data.name, token: t.token };
    ...
    await ctx.db.put({ ...t, needsReauth: r.code === 190, lastError: r.message || JSON.stringify(r) }, { db: "vault" });
    await log(ctx, { op: "token-verify-failed", platform: t.platform, error: r.message || r.egressDenied });
  }
}
...
async function rotateToken(ctx, t) {
  ...
  if (r.ok && r.data.access_token) {
    ...
    await ctx.db.put({ ...t, needsReauth: r.code === 190, lastError: r.message || JSON.stringify(r) }, { db: "vault" });
    await log(ctx, { op: "token-rotate-failed", platform: t.platform, error: r.message || r.egressDenied });
  }
}
...
async function advanceRequest(ctx, req, t) {
  ...
  const text = req.caption || "";
  const save = (patch) =>
    ctx.db.put({ ...req, ...patch, attempts: (req.attempts || 0) + 1, updatedAt: new Date().toISOString() }, { db: "requests" });
  ...
  const fail = async (r, label) => {
    if (platform === "linkedin" || platform === "bsky" ? r.status === 401 : r.code === 190) {
      ...
      await log(ctx, { op: "publish-held", slug: req.slug, error: `token needs re-auth (${label})` });
      return save({ status: req.status });
    }
    return save({ status: "error", error: `${label}: ${r.message || r.egressDenied || r.transport}` });
  };
  if ((req.attempts || 0) >= MAX_ATTEMPTS) {
    return save({ status: "error", error: `gave up after ${MAX_ATTEMPTS} ticks (last: ${req.status})` });
  }
  try {
    ...
    if (platform === "bsky") {
      if (images.length > 0) {
        return save({ status: "error", error: "bsky is text+link-card only for now — image embeds need uploadBlob (v2)" });
      }
      ...
      await log(ctx, { op: "published", slug: req.slug, platform, mediaId: r.data.uri || null, permalink });
      return;
    }
    ...
    if (platform === "linkedin") {
      if (images.length > 0) {
        return save({ status: "error", error: "linkedin is article-link only for now — image posts need the Images API (v2)" });
      }
      ...
      await log(ctx, { op: "published", slug: req.slug, platform, mediaId: r.id || null, permalink });
      return;
    }
    // Facebook Pages: synchronous — no containers, no polling, done in one tick.
    if (platform === "fbpage") {
      if (images.length > 1) return save({ status: "error", error: "multi-image not wired for fbpage yet" });
      let r;
      if (images.length === 1) {
      ...
      const perma = await graph(host, `${V}/${postId}`, { params: { fields: "permalink_url" }, token: t.token });
      await save({ status: "done", mediaId: postId, permalink: perma.ok ? perma.data.permalink_url : null });
      await log(ctx, {
        op: "published",
        slug: req.slug,
        platform,
    ...
    if (text.length > D.maxText) {
      return save({ status: "error", error: `${platform} text is ${text.length} chars (max ${D.maxText})` });
    }
    // Text-only post (Threads only) or single image: one container, then publish.
    if (req.status === "pending" && images.length <= 1) {
      let params;
      if (images.length === 0) {
        if (!D.textOnly) return save({ status: "error", error: "text-only posts are Threads-only" });
        params = D.textOnly(text);
      } else {
        params = D.single(images[0], text);
      }
      const r = await graph(host, `${V}/${D.media(t.igUserId)}`, { method: "POST", params, token: t.token });
    ...
    if (req.status === "pending") {
      ...
      for (const url of images) {
        const r = await graph(host, `${V}/${D.media(t.igUserId)}`, { method: "POST", params: D.child(url), token: t.token });
    ...
    if (req.status === "items-created") {
      ...
      if (platform !== "threads") {
        for (const id of req.children) {
          const r = await graph(host, `${V}/${id}`, { params: { fields: D.statusFields }, token: t.token });
          if (!r.ok) return fail(r, "item status");
          if (D.stat(r.data) === "ERROR")
            return save({ status: "error", error: `item ${id} failed: ${D.detail(r.data) || "no detail"}` });
      ...
      if (!r.ok) {
        if (platform === "threads" && r.code !== 190) {
          await log(ctx, { op: "threads-carousel-retry", slug: req.slug, error: r.message || r.egressDenied });
    ...
    if (req.status === "carousel-created") {
      ...
      if (platform !== "threads") {
        const s = await graph(host, `${V}/${req.carouselId}`, { params: { fields: D.statusFields }, token: t.token });
        if (!s.ok) return fail(s, "container status");
        if (D.stat(s.data) === "ERROR")
          return save({ status: "error", error: `container failed: ${D.detail(s.data) || "no detail"}` });
      ...
      if (!p.ok) {
        if (platform === "threads" && p.code !== 190) {
          ...
          await log(ctx, { op: "threads-publish-retry", slug: req.slug, error: p.message || p.egressDenied });
      ...
      const perma = await graph(host, `${V}/${p.data.id}`, { params: { fields: "permalink" }, token: t.token });
      await save({
        status: "done",
        mediaId: p.data.id,
...
export async function scheduled(event, ctx) {
  ...
  for (const t of vault) {
    ...
    else if (!NO_ROTATE.has(t.platform) && daysSince(t.refreshedAt) > ROTATE_AFTER_DAYS) await rotateToken(ctx, t);
  }
  ...
  for (const t of fresh) {
    ...
    const changed =
      !prev || ["username", "expiresAt", "refreshedAt", "needsReauth", "lastError", "hasToken"].some((k) => prev[k] !== next[k]);
  ...
  for (const req of reqs) {
    ...
    if (!t || !t.token) {
      await ctx.db.put(
        { ...req, status: "error", error: `no ${req.platform || "ig"} token in vault`, updatedAt: new Date().toISOString() },
        { db: "requests" }
      );
      continue;
    }
    ...
    if (held) {
      if (req.heldReason !== held) {
        await ctx.db.put({ ...req, heldReason: held, updatedAt: new Date().toISOString() }, { db: "requests" });
      }
      continue;
    }
