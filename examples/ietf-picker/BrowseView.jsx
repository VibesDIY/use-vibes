  ...
  return (
    <div>
      <div className="mb-1.5 flex flex-wrap gap-1">
        <input
          type="text"
          placeholder="Search sessions or acronyms..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className={`flex-1 min-w-64 p-2 m-0.5 ${c.border} rounded-lg text-lg font-bold ${c.bodyText}`}
        />
        <select
          value={selectedDay}
          onChange={(e) => setSelectedDay(e.target.value)}
          className={`p-2 m-0.5 ${c.border} rounded-lg font-bold bg-white dark:bg-[#21262d] ${c.bodyText}`}
        >
      ...
      {/* Group into per-day sections with headers, matching the My Faves page. Sessions
          carry a meeting-day `day` (4 AM cutoff), so a late social sorts under the
          right day. Ordered by the meeting day order; empty days are skipped. */}
      {(() => {
        const byDay = {};
      ...
      {searchTerm && filteredEvents.length === 0 && (
        <div className="text-center py-3">
          ...
          <p className={c.bodyText}>Try a different session name or group acronym</p>
        </div>
      )}
    </div>
  );
