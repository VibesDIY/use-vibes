...
import { toMeetingDate, meetingDayFor, fmtTime as fmtTimeUtil } from "./festival-utils.js";
...
function GapStrip({ startMs, endMs, allDayEvents, fmtTime }) {
  const count = allDayEvents.filter((e) => {
    const es = toMeetingDate(e.start).getTime();
    const ee = toMeetingDate(e.end).getTime();
  ...
  return (
    <div className="rounded-md m-0.5 px-[7px] py-[5px] bg-white/40 dark:bg-white/10 flex items-center gap-0.5">
      <span className="text-xs font-bold text-[#1f2328]/60 dark:text-[#e6edf3]/60">
        {startStr}–{endStr} · {count} session{count !== 1 ? "s" : ""}
      </span>
    </div>
  );
}
  ...
  return (
    <>
      {days.map((day) => {
        ...
        const allDayEvents = showGaps && allEvents ? allEvents.filter((e) => meetingDayFor(e.start) === day) : [];
        ...
        for (let i = 0; i < daySchedule.length; i++) {
          ...
          const itemStartMs = toMeetingDate(itemStart).getTime();
          const itemEndMs = toMeetingDate(itemEnd).getTime();

          if (showGaps && allDayEvents.length > 0 && i === 0) {
            const earliestEvent = allDayEvents.reduce((min, e) => {
              const t = toMeetingDate(e.start).getTime();
              return t < min ? t : min;
            }, Infinity);
          ...
          if (showGaps && allDayEvents.length > 0) {
            ...
            const nextStartMs = nextItem
              ? toMeetingDate(nextItem.type === "shift" ? shiftStartRaw(nextItem.data) : nextItem.data.start).getTime()
              : null;
            ...
            if (!nextItem) {
              const latestEvent = allDayEvents.reduce((max, e) => {
                const t = toMeetingDate(e.end).getTime();
                return t > max ? t : max;
              }, 0);
            ...
            <div className="space-y-0.5">
              {items.map((entry) => {
                ...
                return (
                  <div
                    key={entry.key}
                    className={item.type === "shift" ? c.schedShift : `rounded-md m-0.5 p-[7px] ${eventCardBg}`}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-0.5 flex-wrap mb-[1px]">
                          ...
                          {isEvent && onToggleFavorite && (
                            <button
                              ...
                              className={`p-[1px] rounded-md m-0.5 text-xs font-bold px-0.5 ${myFavIds && myFavIds.has(item.data.eventId) ? "bg-[#b45309] text-white" : "bg-white dark:bg-[#21262d] text-[#1f2328] dark:text-[#e6edf3]"}`}
                            >
                        ...
                        {isEvent &&
                          (canWrite && saveNote ? (
                            ...
                            <div className={`mt-0.5 p-1.5 bg-[#f1f4f8] dark:bg-[#21262d] rounded-lg m-0.5`}>
