...
const shiftStartOf = (s) => s.start ?? (MEETING_DATES[s.day] && s.startTime ? `${MEETING_DATES[s.day]}T${s.startTime}:00` : null);
const shiftEndOf = (s) => s.end ?? (MEETING_DATES[s.day] && s.endTime ? `${MEETING_DATES[s.day]}T${s.endTime}:00` : null);
  ...
  for (const key in data) {
    ...
    for (const a of assignments) {
      ...
      items.push({
        ...
        end: new Date(startDate.getTime() + (parseDurationMs(a.duration) ?? DEFAULT_SESSION_MS)).toISOString(),
...
const handleSubscription = async (url) => {
  ...
  return new Response(buildFavesCalendar(items, { calName: `@${handle ?? displayName ?? "my"} — IETF ${MEETING_NUMBER} Picks` }), {
    status: 200,
    headers: {
      "content-type": "text/calendar; charset=utf-8",
