...
export default function IetfAgendaPicker() {
  ...
  const {
    ...
    merge: mergeShift,
    reset: resetShift,
  } = useDocument({ type: "shift", day: "Saturday", startTime: "09:00", endTime: "17:00", kind: "Shift", shareWithFriends: false });
  ...
  const favoriteEvents = useMemo(
    () => events.filter((e) => myFavIds.has(e.eventId)).sort((a, b) => toMeetingDate(a.start) - toMeetingDate(b.start)),
    [events, myFavIds]
  );
  ...
  const makeSchedule = (day) => {
    ...
    return [
      ...
      ...sh.map((s) => ({ type: "shift", id: s._id, sort: toMeetingDate(shiftStartRaw(s)), data: s })),
            ...
            <div>
              <a href={MEETING_URL} target="_blank" rel="noopener noreferrer">
                <h1 className={`text-4xl font-black ${c.bodyText} mb-[1px]`}>
                  {superMode ? `SUPER IETF ${MEETING_NUMBER} AGENDA PICKER` : `IETF ${MEETING_NUMBER} AGENDA PICKER`}
                </h1>
              </a>
              <p className={`${c.bodyText} text-base font-bold`}>July 18–24, 2026 · Vienna, Austria</p>
            </div>
          </div>
        ...
        <div className="p-1.5">
          {scheduleLoading ? (
            <div className="flex flex-col items-center justify-center gap-[5px] py-5">
              ...
              <h2 className={`text-3xl font-black text-center ${c.bodyText}`}>Loading the agenda...</h2>
            </div>
          ) : scheduleError ? (
          ...
          {view === "schedule" && (
            <div>
              <div className="flex items-center justify-between flex-wrap gap-0.5 mb-1.5">
                <h2 className={`text-2xl font-black ${c.bodyText}`}>My IETF {MEETING_NUMBER} Schedule</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-1" role="dialog" aria-modal="true">
          <div className="bg-white dark:bg-[#21262d] rounded-lg p-2 max-w-sm w-full shadow-2xl">
            ...
            <p className={`font-bold mb-1.5 ${c.bodyText}`}>
              This link wants to add <span className="text-[#205a9e] dark:text-[#58a6ff]">@{friendConfirm}</span> to your crew.
