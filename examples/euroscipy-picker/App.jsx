...
export default function EuroSciPyPicker() {
  ...
  const { database, useLiveQuery, useDocument } = useFireproof("euroscipy2026", {
    anonymousLocal: true,
  ...
  const getCached = () => {
    const data = localStorage.getItem("euroscipy2026-schedule-cache");
    const ts = +localStorage.getItem("euroscipy2026-schedule-timestamp");
    if (!data || !ts) return null;
    return { data: JSON.parse(data), isStale: Date.now() - ts > 600_000 };
  };
  const setCached = (d) => {
    localStorage.setItem("euroscipy2026-schedule-cache", JSON.stringify(d));
    localStorage.setItem("euroscipy2026-schedule-timestamp", Date.now().toString());
  };
  ...
  const fetchSchedule = async () => {
    ...
    try {
      const res = await fetch("https://pretalx.com/euroscipy-2026/schedule/export/schedule.json");
  ...
  // The pretalx export nests events under days → rooms; flattenPretalx walks that
  // shape into the flat internal event list (guid-keyed, track-colored, day-bucketed
  // with the 4 AM cutoff — the same rule the faves/friends schedules use).
  const ingest = (data) => {
  ...
  // on it (the feed lists the opening weekend with empty rooms today) shouldn't show up
  // in the picker or as an empty section.
  ...
  // Group talks by track — the Tracks tab shows the conference one theme at a time.
  ...
  const connectUrl = `https://vibes.diy/vibe/calendar/euroscipy-picker/?friend=${encodeURIComponent(userId)}`;
      ...
      <div className={`max-w-6xl mx-auto ${c.cardBg} shadow-2xl ${c.border} overflow-hidden`}>
        <div className={`${c.headerBg} ${c.border} p-2.5`}>
          <div className="flex items-start justify-between gap-1 flex-wrap">
            <div>
              <a href="https://www.euroscipy.org/" target="_blank" rel="noopener noreferrer">
                <h1 className={`text-4xl font-black tracking-tight ${c.bodyText} mb-[1px]`}>
                  {superMode ? "Super EuroSciPy 2026 Picker" : "EuroSciPy 2026 Picker"}
                </h1>
              </a>
              <p className={`${c.bodyText} text-base font-bold`}>July 18–23, 2026 · Poland</p>
            </div>
          </div>
          {error && error.includes("cached") && (
            <div className={`mt-0.5 ${c.accentBg} text-[#1a1a1a] px-[3px] py-0.5 rounded-lg text-sm font-bold`}>{error}</div>
          )}
        </div>
          ...
          {view === "schedule" && (
            <div>
              <div className="flex items-center justify-between flex-wrap gap-0.5 mb-1.5">
                <h2 className={`text-2xl font-black ${c.bodyText}`}>My EuroSciPy Schedule</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-1" role="dialog" aria-modal="true">
          <div className="bg-white dark:bg-[#15202b] rounded-2xl p-2 max-w-sm w-full shadow-2xl">
            ...
            <p className={`font-bold mb-1.5 ${c.bodyText}`}>
              This link wants to add <span className="text-[#4B8BBE]">@{friendConfirm}</span> to your crew. You'll see each other's
              schedules.
            </p>
