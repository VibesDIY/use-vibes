// EuroSciPy 2026 Picker backend: serve faves schedules as .ics.
//
// POST /_api/faves.ics  { items: [{ id, title, start, end, location?, url? }] }
//   → 200 text/calendar attachment (euroscipy2026-faves.ics) — one-shot download of
...
//   no ics aggregate). Unlike a handle-keyed URL it is unguessable (a handle
//   in the URL invites swapping in someone else's), shareable on purpose, and
//   revocable (delete the doc; the feed drains). It is still a live feed: new
//   picks flow to every subscriber without re-subscribing, and talk times come
//   from a fresh join against the live pretalx schedule export (platform
//   egress) on every refresh.
//
...
// #3085). The one lane that CAN read the "euroscipy2026" db is `scheduled`
// (runs as the owner in admin mode), so a 1-minute tick aggregates
...
// Handles DST correctly for any date (July is CEST, but don't hardcode +2).
...
// A schedule string (naive conference-local like "2026-07-20T13:00:00", or with an
// explicit offset/Z as the pretalx feed ships) → ICS UTC basic format
...
export const SCHEDULE_URL = "https://pretalx.com/euroscipy-2026/schedule/export/schedule.json";
...
// after isolate boot — the GET answers anchor-only then, never a bogus error.
let subCache = null;
...
export async function scheduled(event, ctx) {
  const docs = await ctx.db.query({ db: "euroscipy2026" });
  const users = new Map();
...
// Fetch the live pretalx schedule export and project the requested event guids
// into items. The frab shape nests events under conference.days[].rooms[roomName];
// an event's start is its `date` (ISO with explicit offset), and when `end` is
// absent we derive it from start + `duration`.
  ...
  for (const day of days) {
    ...
    for (const roomName in rooms) {
      ...
      for (const ev of roomEvents) {
        ...
        let end = typeof ev.end === "string" && ev.end !== "" ? ev.end : "";
        if (end === "") {
          const mins = durationMinutes(ev.duration);
          const startDate = parseToDate(start);
          if (mins !== null && startDate !== null) end = new Date(startDate.getTime() + mins * 60000).toISOString();
        }
        items.push({
          id: `event-${ev.guid}`,
          ...
          location: String(ev.room ?? roomName),
...
const icsUid = (item) => {
  ...
  return `${key.replace(/[^A-Za-z0-9._-]/g, "-")}@euroscipy-picker.vibes.diy`;
};
...
export const buildFavesCalendar = (items, { now, calName } = {}) => {
  ...
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//vibes.diy//euroscipy-picker//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    `X-WR-CALNAME:${escapeIcsText(calName || "My EuroSciPy 2026 Picks")}`,
...
const handleDownload = async (request) => {
  ...
  return new Response(ics, {
    ...
    headers: {
      ...
      "content-disposition": 'attachment; filename="euroscipy2026-faves.ics"',
...
// cold-cache window that would otherwise error into "Validation failed". It
// also gives a zero-faves subscriber something better than an apparently-broken
// empty calendar, and it's real conference info.
const ANCHOR_ITEMS = [
  {
    id: "euroscipy2026-opening",
...
const handleSubscription = async (url) => {
  ...
  return new Response(buildFavesCalendar(items, { calName: `@${handle ?? displayName ?? "my"} — EuroSciPy 2026 Picks` }), {
    status: 200,
    headers: {
      "content-type": "text/calendar; charset=utf-8",
