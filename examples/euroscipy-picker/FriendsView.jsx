    ...
    <div>
      <div className="flex flex-col items-center gap-1 p-2.5 bg-[#dbe7f3] dark:bg-[#101c2a] rounded-2xl m-0.5 mb-1.5">
        ...
        <div className="flex items-center gap-[3px]">
          <button
            onClick={copyLink}
            className={`flex items-center gap-0.5 py-[7px] px-2.5 font-bold rounded-2xl m-0.5 transition-all ${copied ? "bg-[#4B8BBE] text-white" : "bg-white dark:bg-[#15202b] text-[#22303c] dark:text-[#e4edf5] hover:bg-[#dbe7f3] dark:hover:bg-[#101c2a]"}`}
          >
            {copied ? (
              <>
                <svg
        ...
        {friends.length + friendedBy.length > 0 && (
          <div className="flex items-center flex-wrap gap-0.5 mb-1.5">
            <button
              ...
              className={`py-[5px] px-2 font-black rounded-full m-0.5 transition-all ${selectedFriend === ALL_FRIENDS ? "bg-[#FFD43B] text-[#1a1a1a]" : "bg-[#4B8BBE] dark:bg-[#1d3a55] text-white hover:opacity-90"}`}
            >
              All
            </button>
            ...
            <label className={`flex items-center gap-0.5 text-sm font-bold ${c.bodyText} cursor-pointer`}>
              <input
                ...
                className="w-4 h-4 accent-[#FFD43B]"
              />
              include my faves
            </label>
          </div>
      ...
      {selectedFriend && (
        <div id="friend-schedule" className="mb-1.5 scroll-mt-4">
          <div className="flex items-center justify-between mb-1 flex-wrap gap-0.5">
            ...
            <button onClick={() => setSelectedFriend(null)} className={c.btnBlue}>
              Close
            </button>
          </div>
          <ScheduleView
            ...
            emptyMessage={
              selectedFriend === ALL_FRIENDS ? "Your crew hasn't picked any talks yet." : "This friend hasn't picked any talks yet."
            }
            canWrite={false}
