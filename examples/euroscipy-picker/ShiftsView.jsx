...
export default function ShiftsView({
  ...
  return (
    <div>
      <h2 className={`text-2xl font-black mb-1.5 ${c.bodyText}`}>Manage Your Extras</h2>
      {canWrite && (
        <div className={c.shiftForm}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-1">
            ...
            <input
              type="text"
              placeholder="Sprint, Meal, Break, …"
              value={shiftForm.kind || ""}
              onChange={(e) => mergeShift({ kind: e.target.value })}
              className={c.input}
            />
          </div>
          <label className={`mt-1 flex items-center gap-0.5 font-bold ${c.bodyText}`}>
            <input
              type="checkbox"
          ...
          <button onClick={handleSubmit} disabled={submitting} className={`mt-1 ${submitting ? c.btnBlueWorking : c.btnBlue}`}>
            Add Extra
          </button>
        </div>
      )}
      ...
      {shifts.length === 0 && (
        <div className="text-center py-3">
          ...
          <p className={c.bodyText}>Use the form above to add sprints, meals, breaks, or anything else</p>
        </div>
      )}
    </div>
  );
