...
const items = (...overrides) =>
  overrides.length > 0
    ? overrides
    : [{ id: "event-1", title: "Free Threading in Practice", start: "2026-07-20T13:00:00", end: "2026-07-20T14:00:00" }];
...
describe("toIcsUtc — conference-local strings become ICS UTC stamps", () => {
  ...
  it("respects an explicit offset instead of assuming conference time", () => {
    ...
    // The pretalx feed's own form: explicit +02:00.
    expect(toIcsUtc("2026-07-20T13:00:00+02:00")).toBe("20260720T110000Z");
  });
  ...
  it("returns null for garbage, empty, and non-strings", () => {
    expect(toIcsUtc("2026-07-20T:00")).toBe(null); // the known malformed-shift shape
...
describe("parseFavesItems — strict payload validation", () => {
  it("accepts a minimal valid payload and normalizes times to UTC", () => {
    ...
    expect(r.items[0]).toMatchObject({ title: "Free Threading in Practice", start: "20260720T110000Z", end: "20260720T120000Z" });
  });
  ...
  it("keeps location and http(s) url, drops a javascript: url silently", () => {
    ...
    expect(r.ok).toBe(true);
    expect(r.items[0].location).toBe("Room 1.38 (Ground Floor, Turing)");
  ...
  it("normalizes an overnight extra (same-day 22:00 → 01:00) to end the next day", () => {
    ...
    const r = parseFavesItems({ items: [{ title: "Late sprint", start: "2026-07-20T22:00:00", end: "2026-07-20T01:00:00" }] });
    expect(r.ok).toBe(true);
    // 22:00 CEST Jul 20 → 20:00Z Jul 20; 01:00 CEST bumped to Jul 21 → 23:00Z Jul 20.
    expect(r.items[0].start).toBe("20260720T200000Z");
    expect(r.items[0].end).toBe("20260720T230000Z");
  });
  it("rejects zero-duration items and ends more than a day early", () => {
    const zero = parseFavesItems({ items: [{ title: "A", start: "2026-07-20T09:00:00", end: "2026-07-20T09:00:00" }] });
    expect(zero.ok).toBe(false);
    expect(zero.error).toContain("zero duration");
    const wayEarly = parseFavesItems({ items: [{ title: "A", start: "2026-07-20T09:00:00", end: "2026-07-18T09:00:00" }] });
  ...
  it("rejects a blank title and an unparseable time, naming the index", () => {
    ...
    const badTime = parseFavesItems({ items: [{ title: "A", start: "2026-07-20T:00", end: "2026-07-20T14:00:00" }] });
...
describe("buildFavesCalendar — the ICS document", () => {
  ...
  const two = parseFavesItems({
    items: [
      { id: "event-2", title: "Later Talk", start: "2026-07-22T01:00:00", end: "2026-07-22T02:00:00" },
      {
        id: "event-1",
        title: "Früh; und, spät",
        start: "2026-07-20T13:00:00",
  ...
  it("has the calendar envelope and metadata", () => {
    ...
    expect(ics).toContain("X-WR-CALNAME:My EuroSciPy 2026 Picks\r\n");
    expect(ics).toContain("X-WR-TIMEZONE:Europe/Warsaw\r\n");
    expect(ics.trimEnd().endsWith("END:VCALENDAR")).toBe(true);
  });
  it("emits events sorted by start time regardless of input order", () => {
    // 2026-07-22T01:00 CEST crosses midnight UTC → 20260721T230000Z, and it must come second.
    expect(ics.indexOf("20260720T110000Z")).toBeLessThan(ics.indexOf("20260721T230000Z"));
    expect(ics.indexOf("20260721T230000Z")).toBeGreaterThan(-1);
  });
  it("stamps stable UIDs from the item id", () => {
    expect(ics).toContain("UID:event-1@euroscipy-picker.vibes.diy");
    expect(ics).toContain("UID:event-2@euroscipy-picker.vibes.diy");
  });
  it("escapes SUMMARY text and carries LOCATION/URL", () => {
    expect(ics).toContain("SUMMARY:Früh\\; und\\, spät");
    expect(ics).toContain("LOCATION:Room 1.38 (Ground Floor\\, Turing)");
    expect(ics).toContain("URL:https://pretalx.com/euroscipy-2026/talk/37RGPY/");
  });
...
// The frab shape the pretalx export actually returns: conference.days[] each with
// a rooms map of roomName → [event]; events keyed by guid, `date` carries the
// explicit +02:00 offset, and one event below has no ISO `end` (duration only).
const FEED = {
  schedule: {
    conference: {
        ...
        { index: 1, date: "2026-07-18", rooms: {} },
        {
          ...
          rooms: {
            "Room 1.38 (Ground Floor, Turing)": [
              {
                guid: "aaaa1111-0000-0000-0000-000000000101",
                date: "2026-07-20T13:00:00+02:00",
                end: "2026-07-20T13:30:00+02:00",
                duration: "00:30",
                room: "Room 1.38 (Ground Floor, Turing)",
                url: "https://pretalx.com/euroscipy-2026/talk/AAA101/",
                title: "Free Threading in Practice",
                track: "Computational Tools and Scientific Python Infrastructure",
              },
              {
                guid: "bbbb2222-0000-0000-0000-000000000102",
                date: "2026-07-21T11:00:00+02:00",
                end: "2026-07-21T12:00:00+02:00",
                duration: "01:00",
                room: "Room 1.38 (Ground Floor, Turing)",
                url: "https://pretalx.com/euroscipy-2026/talk/BBB102/",
                title: "Skills & Games with Arrays",
                track: "Community, Education, and Outreach",
              },
            ],
            "Room 1.19 (Ground Floor, Shannon)": [
              {
                // No ISO `end` — the join must derive it from date + duration.
                guid: "cccc3333-0000-0000-0000-000000000201",
                date: "2026-07-20T23:00:00+02:00",
                duration: "01:30",
                room: "Room 1.19 (Ground Floor, Shannon)",
                url: "https://pretalx.com/euroscipy-2026/talk/CCC201/",
                title: "Night Sprint",
                track: "Tools",
              },
            ],
...
const G101 = "aaaa1111-0000-0000-0000-000000000101";
const G102 = "bbbb2222-0000-0000-0000-000000000102";
const G201 = "cccc3333-0000-0000-0000-000000000201";

// The db docs the scheduled aggregation tick sees (admin-lane read of the
// access-fn-bound db — the one lane allowed to read it).
const DB_DOCS = [
  { _id: `favorite-Alice-${G101}`, type: "favorite", userId: "Alice", eventId: G101 },
  { _id: `favorite-alice-${G201}`, type: "favorite", userId: "alice", eventId: G201 },
  { _id: `favorite-bob-${G102}`, type: "favorite", userId: "bob", eventId: G102 },
  {
    _id: "shift-1",
    type: "shift",
    userId: "alice",
  ...
  { _id: `note-alice-${G101}`, type: "note", userId: "alice", eventId: G101, notes: "PRIVATE NOTE" },
...
describe("fetch handler — GET /faves.ics?t=<token> (subscription lane)", () => {
  ...
  it("serves the never-empty anchor-only calendar before the first aggregation tick", async () => {
    ...
    expect(body).toContain("SUMMARY:EuroSciPy 2026");
    expect(body).not.toContain("Free Threading"); // faves arrive with the tick
  });
  ...
  it("serves a user's CURRENT faves: db-aggregated guids joined live against the feed", async () => {
    ...
    expect(body).toContain("SUMMARY:Free Threading in Practice"); // alice faved 101 (case-folded handle)
    expect(body).toContain("SUMMARY:Night Sprint"); // alice faved 201
    expect(body).not.toContain("Skills & Games"); // 102 is bob's
    ...
    expect(body).toContain("LOCATION:Room 1.38 (Ground Floor\\, Turing)");
    expect(body).toContain(`UID:event-${G101}@euroscipy-picker.vibes.diy`); // stable across refreshes
    expect(body).toContain("SUMMARY:EuroSciPy 2026"); // the always-present anchor event
    expect(body).toContain("X-WR-CALNAME:@alice — EuroSciPy 2026 Picks");
    expect(body).toContain("REFRESH-INTERVAL;VALUE=DURATION:PT6H");
  });

  it("derives a feed event's missing end from its duration", async () => {
    feedOk();
    await tick();
    const body = await (await icsFetch(req(`/faves.ics?t=${T_ALICE}`), {})).text();
    // Night Sprint: 23:00 +02:00 (21:00Z) plus 1h30 → DTEND 22:30Z.
    expect(body).toContain("DTSTART:20260720T210000Z");
    expect(body).toContain("DTEND:20260720T223000Z");
  });
  ...
  it("serves fave-less holders and pre-tick tokens alike: valid anchor-only, never an error", async () => {
    ...
    expect(body).toContain("X-WR-CALNAME:@jchris — EuroSciPy 2026 Picks");
  ...
  it("derives legacy shift times from day + startTime/endTime", async () => {
    ...
    expect(body).toContain("SUMMARY:Registration desk");
...
describe("fetch handler — POST /faves.ics", () => {
  ...
  it("returns a text/calendar attachment for a valid payload", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toBe("text/calendar; charset=utf-8");
    expect(res.headers.get("content-disposition")).toBe('attachment; filename="euroscipy2026-faves.ics"');
    expect(res.headers.get("cache-control")).toBe("no-store");
    const body = await res.text();
    expect(body).toContain("BEGIN:VEVENT");
    expect(body).toContain("SUMMARY:Free Threading in Practice");
    expect(body).toContain("DTSTART:20260720T110000Z"); // conference-local converted to UTC
  });
