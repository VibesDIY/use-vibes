...
const items = (...overrides) =>
  overrides.length > 0
    ? overrides
    : [{ id: "event-1", title: "Built to Spill", start: "2026-07-31T13:00:00", end: "2026-07-31T14:00:00" }];
...
describe("parseFavesItems — strict payload validation", () => {
  it("accepts a minimal valid payload and normalizes times to UTC", () => {
    ...
    expect(r.items[0]).toMatchObject({ title: "Built to Spill", start: "20260731T200000Z", end: "20260731T210000Z" });
  });
  ...
  it("normalizes an overnight extra (same-day 22:00 → 01:00) to end the next day", () => {
    ...
    const r = parseFavesItems({ items: [{ title: "Late shift", start: "2026-07-30T22:00:00", end: "2026-07-30T01:00:00" }] });
    expect(r.ok).toBe(true);
  ...
  it("rejects zero-duration items and ends more than a day early", () => {
    const zero = parseFavesItems({ items: [{ title: "A", start: "2026-07-30T09:00:00", end: "2026-07-30T09:00:00" }] });
    expect(zero.ok).toBe(false);
    expect(zero.error).toContain("zero duration");
    const wayEarly = parseFavesItems({ items: [{ title: "A", start: "2026-07-30T09:00:00", end: "2026-07-28T09:00:00" }] });
  ...
  it("rejects a blank title and an unparseable time, naming the index", () => {
    const bad = parseFavesItems({ items: [{ title: "  ", start: "2026-07-31T13:00:00", end: "2026-07-31T14:00:00" }] });
    expect(bad.ok).toBe(false);
    expect(bad.error).toContain("items[0].title");
    const badTime = parseFavesItems({ items: [{ title: "A", start: "2026-07-30T:00", end: "2026-07-31T14:00:00" }] });
...
describe("buildFavesCalendar — the ICS document", () => {
  ...
  const two = parseFavesItems({
    items: [
      { id: "event-2", title: "Later Act", start: "2026-08-01T20:00:00", end: "2026-08-01T21:00:00" },
      {
        id: "event-1",
        title: "Früh; und, spät",
        start: "2026-07-31T13:00:00",
  ...
  it("emits URL as a URI, never TEXT-escaping its commas/semicolons", () => {
    const withPunct = parseFavesItems({
      items: [{ id: "e", title: "A", start: "2026-07-31T13:00:00", end: "2026-07-31T14:00:00", url: "https://x.com/a,b;c" }],
    }).items;
...
const FEED = {
  12: {
    ...
    events: [
      ...
      { id: 102, title: "Skills &amp; Games", start: "2026-08-01 11:00:00", end: "2026-08-01 12:00:00" },
    ],
  },
  13: { title: "Galaxy Barn", events: [{ id: 201, title: "Night Act", start: "2026-08-01 23:00:00", end: "2026-08-02 00:30:00" }] },
};
...
describe("fetch handler — POST /faves.ics", () => {
  ...
  it("400s malformed JSON and invalid payloads", async () => {
    ...
    const bad = await icsFetch(post({ items: [{ title: "A", start: "junk", end: "2026-07-31T14:00:00" }] }), {});
  ...
  it("returns a text/calendar attachment for a valid payload", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toBe("text/calendar; charset=utf-8");
    expect(res.headers.get("content-disposition")).toBe('attachment; filename="pickathon-faves.ics"');
