    ...
    <div>
      <div className="mb-1.5 flex flex-wrap gap-1">
        ...
        <select
          ...
          className={`p-2 m-0.5 ${c.border} rounded-2xl font-bold bg-white dark:bg-[#262031] ${c.bodyText}`}
        >
      ...
      {/* Group into per-day sections with headers, matching the My Faves page. Events
          carry a conference-day `day` (4 AM cutoff), so a post-midnight social sorts
          under the right day. Ordered by the conference day order; empty days are skipped. */}
      {(() => {
        const byDay = {};
                  ...
                  >
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-1">
                      <div className="flex-1">
                        ...
                        {event.speakers && (
                          <p className="text-xs font-bold text-[#2a2a2a]/70 dark:text-[#ece9f1]/70">{event.speakers}</p>
                        )}
                      ...
                      <div className="flex gap-0.5">
                        ...
                        <a href={event.url} target="_blank" rel="noopener noreferrer" className={c.linkBtn} title="View talk page">
                          <svg
                            width="20"
                            height="20"
                            viewBox="0 0 24 24"
      ...
      {searchTerm && filteredEvents.length === 0 && (
        <div className="text-center py-3">
          <h3 className={`text-2xl font-black mb-0.5 ${c.bodyText}`}>No talks found</h3>
          <p className={c.bodyText}>Try a different talk title or speaker name</p>
        </div>
      )}
    </div>
  );
