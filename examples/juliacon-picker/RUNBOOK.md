# JuliaCon 2026 Picker — Update Runbook

Live URL: https://vibes.diy/vibe/calendar/juliacon-picker
Super mode (once live): https://vibes.diy/vibe/calendar/juliacon-picker?super=true

## Edit → Push

```bash
cd vibes/juliacon-picker
# edit App.jsx
npx vibes-diy push --vibe calendar/juliacon-picker
```

That's it. `push` deploys `App.jsx` to `calendar/juliacon-picker` and prints the live URL.

## Pull current live version

```bash
cd vibes/juliacon-picker
npx vibes-diy pull calendar/juliacon-picker
```
...
npx vibes-diy db put --vibe calendar/juliacon-picker --db juliacon2026 \
...
- **📅 Download .ics** — one-shot: the client POSTs its faves + extras to
  ...
  (`juliacon2026-faves.ics`). Works for anonymous (local-only) faves too, since
...
- **🔁 Subscribe on iPhone** — persistent: a `webcal://…/_api/faves.ics?t=<token>&n=<handle>`
  ...
  a handle-keyed form would invite swapping in someone else's handle — and
...
`scheduled` lane (owner, admin mode) can read the `juliacon2026` db. So `backend.js`
...
(a hard-coded "JuliaCon 2026" opening event rides in every response, so the feed
...
and `schedule.test.js` (pretalx flattening + item flattening).

## Schedule data

Fetched client-side from
`https://pretalx.com/juliacon-2026/schedule/export/schedule.json` (CORS `*`,
~1.1 MB, frab/pretalx shape: `schedule.conference.days[]`, each day a `rooms`
map of room name → event array) and cached in `localStorage` for 10 minutes.
`flattenPretalx` in `festival-utils.js` produces the flat event list — the
pretalx `guid` is the stable eventId, an explicit `end` is kept and a missing
one is derived from `duration` ("HH:MM"/"H:MM"), a null `track` defaults to
"General", and events with an unparseable `date` are skipped. All times
stored/displayed in `Europe/Berlin`. Conference days run Monday 2026-08-10
through Saturday 2026-08-15 with the 4 AM night cutoff (the same window the
feed's own `day_start` uses).

## Common edits

| Task                    | Where                                                                                                           |
| ----------------------- | --------------------------------------------------------------------------------------------------------------- |
| Change conference dates | `FESTIVAL_2026.dates` (+ the mirrored `FESTIVAL_DATES` in `backend.js`)                                         |
