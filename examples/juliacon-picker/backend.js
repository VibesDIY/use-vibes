// JuliaCon 2026 Picker backend: serve faves schedules as .ics.
//
// POST /_api/faves.ics  { items: [{ id, title, start, end, location?, url? }] }
//   → 200 text/calendar attachment (juliacon2026-faves.ics) — one-shot download of
...
//   talk times come from a fresh join against the live pretalx schedule feed
//   (platform egress) on every refresh.
//
...
// #3085). The one lane that CAN read the "juliacon2026" db is `scheduled` (runs
...
// Same offset trick as festival-utils.js: format the instant in the conference
// zone, re-read it as if it were UTC, and the difference is the zone offset.
// Handles DST correctly for any date (the conference is CEST, but don't hardcode +2).
...
// A schedule string (naive conference-local like "2026-08-11T10:00:00", or with an
// explicit offset/Z like the pretalx feed's) → ICS UTC basic format
// "20260811T080000Z", or null if it doesn't parse. UTC-basic strings sort
...
// loose enough that a maximal real schedule (every talk + every shift) fits.
...
export const validateFavesItem = (it) => {
  ...
  // to end the next day. (+24h in absolute time; the conference dates never
  // straddle a DST change, so local wall time is preserved.)
...
export const SCHEDULE_URL = "https://pretalx.com/juliacon-2026/schedule/export/schedule.json";

// Conference day → calendar date, for legacy shift docs stored without absolute
...
// after isolate boot — the GET answers anchor-only then, never an error.
let subCache = null;
...
export async function scheduled(event, ctx) {
  const docs = await ctx.db.query({ db: "juliacon2026" });
  const users = new Map();
...
// A pretalx duration ("HH:MM", occasionally "H:MM") added to a start string →
// ISO end, for the rare event row that lacks an explicit `end`. Returns "" when
// either piece doesn't parse — the lenient lane drops the row, never the feed.
const endFromDuration = (start, duration) => {
  const m = /^(\d{1,2}):(\d{2})$/.exec(typeof duration === "string" ? duration : "");
  const d = parseToDate(start);
  if (!m || d === null) return "";
  return new Date(d.getTime() + (+m[1] * 60 + +m[2]) * 60000).toISOString();
};

// Fetch the live pretalx schedule feed and project the requested event guids
// into items. The frab shape: schedule.conference.days[] each carrying a
// `rooms` map of room name → event array; the room name is the location.
...
export const fetchScheduleItems = async (ids) => {
  ...
  const days =
    data && data.schedule && data.schedule.conference && Array.isArray(data.schedule.conference.days)
      ? data.schedule.conference.days
      : [];
  const items = [];
  for (const day of days) {
    const rooms = day && day.rooms && typeof day.rooms === "object" ? day.rooms : {};
    for (const roomName in rooms) {
      const events = Array.isArray(rooms[roomName]) ? rooms[roomName] : [];
      for (const ev of events) {
        if (!wanted.has(String(ev.guid))) continue;
        const start = String(ev.date ?? "");
        items.push({
          id: `event-${ev.guid}`,
          title: String(ev.title ?? ""),
          start,
          end: ev.end ? String(ev.end) : endFromDuration(start, ev.duration),
          location: roomName,
...
const icsUid = (item) => {
  ...
  return `${key.replace(/[^A-Za-z0-9._-]/g, "-")}@juliacon-picker.vibes.diy`;
};
...
export const buildFavesCalendar = (items, { now, calName } = {}) => {
  ...
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//vibes.diy//juliacon-picker//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    `X-WR-CALNAME:${escapeIcsText(calName || "My JuliaCon 2026 Picks")}`,
...
const handleDownload = async (request) => {
  ...
  return new Response(ics, {
    ...
    headers: {
      ...
      "content-disposition": 'attachment; filename="juliacon2026-faves.ics"',
...
// empty calendar, and it's real conference info.
const ANCHOR_ITEMS = [
  {
    id: "juliacon2026-opening",
    title: "JuliaCon 2026",
...
// a popular feed doesn't hammer the pretalx join.
const handleSubscription = async (url) => {
  ...
  return new Response(buildFavesCalendar(items, { calName: `@${handle ?? displayName ?? "my"} — JuliaCon 2026 Picks` }), {
    status: 200,
    headers: {
      "content-type": "text/calendar; charset=utf-8",
