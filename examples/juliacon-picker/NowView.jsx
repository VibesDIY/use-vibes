    ...
    <div className={`rounded-[16px] m-0.5 p-2 shadow-lg ${eventCardBg}`} style={eventCardStyle(event)}>
      <div className="flex justify-between items-start gap-[3px] flex-wrap">
        <div className="flex-1">
          ...
          {event.speakers && <p className="text-xs font-bold text-[#2a2a2a]/70 dark:text-[#ece9f1]/70">{event.speakers}</p>}
...
export default function NowView({ nowSets, nextSets, nowTick, myFavIds, friendFavIds, canWrite, toggleFavorite, c }) {
  return (
    <div>
      ...
      {nowSets.length === 0 ? (
        <div className="mb-1.5 p-2 bg-white dark:bg-[#262031] rounded-2xl m-0.5 ">
      ...
      {nextSets.length === 0 ? (
        <div className="p-2 bg-white dark:bg-[#262031] rounded-2xl m-0.5 ">
