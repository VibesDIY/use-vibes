...
function GapStrip({ startMs, endMs, allDayEvents, fmtTime }) {
  ...
  return (
    <div className=" m-0.5  px-[7px] py-[5px] bg-white/10 flex items-center gap-0.5">
      <span className="text-xs font-bold text-[#FBF6EF]/85">
              ...
              {items.map((entry) => {
                ...
                return (
                  <div
                    key={entry.key}
                    className={
                      item.type === 'shift' ? c.schedShift : ` m-0.5 p-[7px] ${eventCardBg}`
                    }
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-0.5 flex-wrap mb-[1px]">
                          ...
                          {isEvent && onToggleFavorite && (
                            <button
                              ...
                              className={`p-[1px]  m-0.5  text-xs font-bold px-0.5 ${myFavIds && myFavIds.has(item.data.eventId) ? 'bg-[#C8613C] text-white' : 'bg-[#403D31] text-[#FBF6EF]'}`}
                            >
                              <FavMark filled={!!(myFavIds && myFavIds.has(item.data.eventId))} />
                            </button>
                          )}
                        </div>
                        ...
                        {isEvent &&
                          (canWrite && saveNote ? (
                            ...
                            <div className={`mt-0.5 p-1.5 bg-[#EEE] dark:bg-[#35342a]  m-0.5 `}>
                              <p className={`text-sm font-bold ${c.bodyText}`}>
