...
function EventCard({ event, isMine, isFriendPick, canWrite, toggleFavorite, c, showDate }) {
  ...
  return (
    <div className={` m-0.5 p-2 shadow-lg ${eventCardBg}`} style={eventCardStyle(event)}>
    ...
    <div>
      <div className="flex items-center justify-between mb-1.5 flex-wrap gap-[3px]">
        ...
        <p className={`text-sm font-bold ${c.bodyText}`}>
          ...
          <span className="hinter-script text-lg align-baseline">festival time</span>
        </p>
      </div>
      ...
      {nextSets.length === 0 ? (
        <div className="p-2 bg-[#403D31]  m-0.5 ">
