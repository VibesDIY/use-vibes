...
export default function FriendsView({
  ...
  if (!socialReady) {
    return (
      <div className="mb-1.5 p-2.5 bg-[#403D31]  m-0.5">
  ...
  return (
    <div>
      <div className="flex flex-col items-center gap-1 p-2.5 bg-[#403D31]  m-0.5  mb-1.5">
        <div className="flex items-center gap-0.5 flex-wrap justify-center">
          <p className={`text-lg font-bold ${c.bodyText}`}>
            Share this link so people can follow you
          </p>
        </div>
        <div className="bg-[#403D31]  m-0.5  p-2">
          <img src={qrSrc} alt="Follow-me QR code" width="320" height="320" />
        </div>
        <div className="flex items-center gap-[3px]">
          <button
            onClick={copyLink}
            className={`flex items-center gap-0.5 py-[7px] px-2.5 font-bold  m-0.5  transition-all ${copied ? 'bg-[#3BB683] text-white' : 'bg-[#403D31] text-[#FBF6EF] hover:bg-[#4f4b3b]'}`}
          >
            {copied ? (
              <>
                <svg
        ...
        <div className="mb-1.5 p-2.5 bg-[#403D31]  m-0.5 ">
          ...
          <div className="flex flex-wrap gap-[3px]">
            {requests.map((r) => (
              <div
                key={`req-${r.handle}`}
                className="flex items-center gap-0.5 p-0.5  m-0.5 bg-[#403D31]"
              >
                ...
                <button
                  ...
                  disabled={busy.has(r.handle)}
                  className="py-[1px] px-1  m-0.5 text-xs font-bold bg-[#403D31] text-[#FBF6EF] hover:bg-[#B22222] hover:text-white transition-all"
        ...
        {followingActive.length > 0 && (
          <div className="flex items-center flex-wrap gap-0.5 mb-1.5">
            <button
              ...
              className={`py-[5px] px-2 font-black  m-0.5 transition-all ${selectedFriend === ALL_FRIENDS ? 'bg-[#C8613C] text-white' : 'bg-[#3BB683]  text-white hover:opacity-90'}`}
            >
              All
            </button>
            ...
            >
              <input
                ...
                className="w-4 h-4 accent-[#C8613C]"
              />
              include my faves
            </label>
          </div>
        ...
        {following.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-[3px] mb-1.5">
            {followingActive.map((f) => (
              <div
                key={`fw-${f.handle}`}
                className={`flex items-center gap-0.5 p-0.5  m-0.5  transition-all ${selectedFriend === f.handle ? 'bg-[#C8613C]' : 'bg-[#403D31]'}`}
              >
                <button
                ...
                <button
                  ...
                  className="px-0.5 py-[1px]  m-0.5 text-xs font-bold bg-[#403D31] text-[#FBF6EF] hover:bg-[#B22222] hover:text-white transition-all"
                  title="Unfollow"
                >
                  ×
                </button>
            ...
            {followingRequested.map((f) => (
              <div
                key={`fw-${f.handle}`}
                className="flex items-center gap-0.5 p-0.5  m-0.5 bg-[#403D31] opacity-80"
        ...
        {followers.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-[3px]">
            {followers.map((f) => (
              <div
                key={`fb-${f.handle}`}
                className="flex items-center gap-0.5 p-0.5  m-0.5 bg-[#3BB683] "
              >
                <ViewerTag userHandle={f.handle} />
                {canWrite && !followingSet.has(f.handle) && (
                  <button
                    ...
                    disabled={busy.has(f.handle)}
                    className="py-[1px] px-1  m-0.5 text-xs font-bold bg-[#403D31] text-[#FBF6EF] hover:bg-[#C8613C] hover:text-white transition-all"
                  >
                    Follow back
                  </button>
                )}
