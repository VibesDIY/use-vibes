...
describe('festival-config ⇄ backend.js', () => {
  ...
  it('keeps both lanes on the same clock', () => {
    // A tz split between the client and the isolate shifts every .ics event by
    // hours against what the app shows, and nothing else would fail.
    expect(FESTIVAL_TZ).toBe(FESTIVAL.tz);
  });
