...
export const profileEmptyMessage = ({ isSelf, sharingOff, followState, signedIn, picksPending }) => {
  // A follow that just went active grants the read INSTANTLY, but the picks it
  // unlocks still have to replicate into this device's store — a beat or two on a
  // festival network. That window reads as zero docs, and the honest-looking
  // "they haven't shared any yet" is simply false there: it names a cause the
  // viewer can't check and can't fix, and it's the first thing they see after
  // following someone. Pending is its own state, never empty.
  if (picksPending) return 'Getting their picks…';
  if (isSelf)
    return sharingOff
