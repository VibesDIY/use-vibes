...
describe('migrateFestivalDoc', () => {
  it('re-keys a favorite onto the signed-in handle', () => {
    const out = migrateFestivalDoc(
  ...
  it('drops a shift _id — a shift has no natural key, so it gets a fresh one', () => {
    const out = migrateFestivalDoc({ _id: 'old', type: 'shift', day: 'Friday' }, 'alice');
  ...
  it('re-stamps any other doc without touching its id', () => {
    const out = migrateFestivalDoc({ _id: 'x', type: 'caltoken', token: 't' }, 'alice');
    expect(out).toEqual({ _id: 'x', type: 'caltoken', token: 't', userId: 'alice' });
  });

  it('is idempotent for an already-migrated favorite', () => {
    const once = migrateFestivalDoc({ type: 'favorite', eventId: 'e1' }, 'alice');
    expect(migrateFestivalDoc(once, 'alice')).toEqual(once);
  });
});
