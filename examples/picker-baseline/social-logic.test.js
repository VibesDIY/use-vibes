...
describe('profileEmptyMessage', () => {
  ...
  it('reports a still-replicating read as pending, never as empty', () => {
    const msg = profileEmptyMessage({
      isSelf: false,
      followState: 'active',
      signedIn: true,
      picksPending: true,
    });
    expect(msg).toBe('Getting their picks…');
    // The bug this pins: a just-followed profile claiming they have shared nothing
    // while their picks are still on the wire.
    expect(msg).not.toMatch(/haven't shared/i);
  });
