...
// Dates come from the instantiated config, so these assertions survive a
// festival swap: the behaviour under test is the day table, not any one year.
const D = FESTIVAL.dates;
const DAY1 = FESTIVAL.dayOrder[0];
const DAY2 = FESTIVAL.dayOrder[1];
const LAST = FESTIVAL.dayOrder[FESTIVAL.dayOrder.length - 1];
import {
...
describe('getDateForDay', () => {
  it('prefers the canonical festival date over any event start', () => {
    const events = eventsFromScheduleDocs([doc({ start: `${D[LAST]}T01:00:00` })]);
    expect(getDateForDay(DAY2, events)).toBe(D[DAY2]);
  });
  ...
  it('falls back to fallbackStart + day index when nothing else knows', () => {
    expect(getDateForDay('Wednesday', [])).toBe(FESTIVAL.fallbackStart.slice(0, 10));
  });
});
...
describe('displayDaysFor', () => {
  it('lists only days with content, in festival order', () => {
    const events = eventsFromScheduleDocs([
      doc({ eventId: 'a', start: `${D[LAST]}T12:00:00` }),
      doc({ eventId: 'b', start: `${D[DAY1]}T12:00:00` }),
    ]);
    expect(displayDaysFor(events, [])).toEqual([DAY1, LAST]);
  });
...
describe('makeShiftBounds', () => {
  ...
  it('composes day + time when neither is stored', () => {
    const { shiftStartRaw } = makeShiftBounds([]);
    expect(shiftStartRaw({ day: LAST, startTime: '10:30' })).toBe(`${D[LAST]}T10:30:00`);
  });
});
