...
//
//   vibes-diy db put --db <dbName> '{"_id":"0-load-shed","type":"loadshed","level":"read-only"}'
//
...
//
// The digit-leading `_id` is historical — it was chosen to sort ahead of every
// id this app mints so the doc would ride inside a capped whole-db read. The
// backend reads it by id now, so the name means nothing; it stays because
// renaming a live doc's `_id` is a data migration. access.js makes it owner-only
// WRITE but world-READABLE — see the comment there for why both halves matter.
//
