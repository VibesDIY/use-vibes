// State of the Map 2026 Picker backend: serve faves schedules as .ics.
//
// POST /_api/faves.ics  { items: [{ id, title, start, end, location?, url? }] }
//   → 200 text/calendar attachment (sotm2026-faves.ics) — one-shot download of
...
//   talk times come from a fresh join against the live pretalx schedule
...
// #3085). The one lane that CAN read the "sotm2026" db is `scheduled` (runs
...
// A schedule string (naive conference-local like "2026-08-28T13:00:00", or with an
// explicit offset/Z) → ICS UTC basic format "20260828T110000Z", or null if it
...
export const validateFavesItem = (it) => {
  ...
  // to end the next day. (+24h in absolute time; conference dates never straddle
...
export const SCHEDULE_URL = "https://pretalx.com/sotm2026/schedule/export/schedule.json";
...
export async function scheduled(event, ctx) {
  const docs = await ctx.db.query({ db: "sotm2026" });
  const users = new Map();
...
// pretalx duration is "HH:MM"; some exports omit an event's `end`, so recover it
// from date + duration. Returns null when neither field is usable.
...
// Fetch the live schedule feed and project the requested event guids into items.
// The pretalx/frab export nests events per day per room under
// schedule.conference.days[].rooms{}. MUST call globalThis.fetch — bare `fetch`
// here resolves to this module's own exported handler, not the global (module
// scope shadows the isolate global).
export const fetchScheduleItems = async (ids) => {
  ...
  for (const day of days) {
    const rooms = day && day.rooms ? day.rooms : {};
    for (const room in rooms) {
      ...
      for (const ev of events) {
        ...
        const end = ev.end ? String(ev.end) : endFromDuration(start, ev.duration);
        if (end === null) continue; // no recoverable end — sanitize would drop it anyway
        items.push({
          id: `event-${ev.guid}`,
          title: String(ev.title ?? ""),
          start,
          end,
          location: String(ev.room ?? room),
...
const icsUid = (item) => {
  ...
  return `${key.replace(/[^A-Za-z0-9._-]/g, "-")}@sotm-picker.vibes.diy`;
};
...
export const buildFavesCalendar = (items, { now, calName } = {}) => {
  ...
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//vibes.diy//sotm-picker//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    `X-WR-CALNAME:${escapeIcsText(calName || "My State of the Map 2026 Picks")}`,
...
const handleDownload = async (request) => {
  ...
  return new Response(ics, {
    ...
    headers: {
      ...
      "content-disposition": 'attachment; filename="sotm2026-faves.ics"',
...
// calendar, and it's real conference info.
const ANCHOR_ITEMS = [
  {
    id: "sotm2026-opening",
...
const handleSubscription = async (url) => {
  ...
  return new Response(buildFavesCalendar(items, { calName: `@${handle ?? displayName ?? "my"} — State of the Map 2026 Picks` }), {
    status: 200,
    headers: {
      "content-type": "text/calendar; charset=utf-8",
