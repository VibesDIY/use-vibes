...
export const c = {
  ...
  input: "p-[7px] m-0.5 rounded-xl font-bold text-[#2b3a24] dark:text-[#e9f0e3] bg-white dark:bg-[#1b2913]",
  navBtn: (active) =>
  ...
  btnAccent: "bg-[#2d6a8f] text-white font-bold py-[7px] px-2.5 rounded-2xl m-0.5 hover:opacity-90 transition-all",
  btnCyan: "bg-[#4c7a34] dark:bg-[#1c3012] text-white font-bold py-[7px] px-2.5 rounded-2xl m-0.5 hover:opacity-90 transition-all",
  // Same button, in-flight: color-only cue (water blue) while the write lands. Copy is unchanged.
  btnCyanWorking: "bg-[#2d6a8f] text-white font-bold py-[7px] px-2.5 rounded-2xl m-0.5 opacity-90 transition-all cursor-wait",
  badge: "bg-[#2d6a8f] text-white px-[3px] py-[1px] rounded-full text-sm font-bold m-0.5",
  favToggleOn: "p-[7px] rounded-2xl m-0.5 font-bold transition-all bg-[#2d6a8f] text-white hover:opacity-90",
  favToggleOff:
...
export const lineupTag = (event) => {
  ...
  return { label, color: event.lineup?.color || "#4c7a34", textColor: event.lineup?.textColor || "#fff" };
};
