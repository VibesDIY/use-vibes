    ...
    <div>
      <div className="mb-1.5 flex flex-wrap gap-1">
        ...
        <select
          ...
          className={`p-2 m-0.5 ${c.border} rounded-2xl font-bold bg-white dark:bg-[#1b2913] ${c.bodyText}`}
        >
      ...
      {/* Group into per-day sections with headers, matching the My Faves page. Events
          carry a conference-day `day` (4 AM cutoff), so a late social sorts under the
          right day. Ordered by the conference day order; empty days are skipped. */}
      {(() => {
        const byDay = {};
      ...
      {searchTerm && filteredEvents.length === 0 && (
        <div className="text-center py-3">
          ...
          <p className={c.bodyText}>Try searching for a different talk title</p>
        </div>
      )}
    </div>
  );
