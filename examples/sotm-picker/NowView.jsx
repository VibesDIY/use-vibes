    ...
    <div className={`rounded-[16px] m-0.5 p-2 shadow-lg ${eventCardBg}`} style={eventCardStyle(event)}>
      <div className="flex justify-between items-start gap-[3px] flex-wrap">
        <div className="flex-1">
          ...
          {event.speakers && <p className={`text-sm font-bold ${c.bodyText}`}>{event.speakers}</p>}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5 flex-wrap gap-[3px]">
        ...
        <p className={`text-sm font-bold ${c.bodyText}`}>
          {new Date(nowTick).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", timeZone: FESTIVAL_TZ })} Paris time
        </p>
      </div>

      {nowSets.length === 0 ? (
        <div className="mb-1.5 p-2 bg-white dark:bg-[#1b2913] rounded-2xl m-0.5 ">
          <p className={`font-bold ${c.bodyText}`}>No talks are on right now.</p>
        </div>
      ) : (
      ...
      {nextSets.length === 0 ? (
        <div className="p-2 bg-white dark:bg-[#1b2913] rounded-2xl m-0.5 ">
