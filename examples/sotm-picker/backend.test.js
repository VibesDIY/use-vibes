...
const items = (...overrides) =>
  overrides.length > 0
    ? overrides
    : [{ id: "event-guid-101", title: "Opening", start: "2026-08-28T09:30:00+02:00", end: "2026-08-28T09:50:00+02:00" }];
...
describe("parseFavesItems — strict payload validation", () => {
  it("accepts a minimal valid payload and normalizes times to UTC", () => {
    ...
    expect(r.items[0]).toMatchObject({ title: "Opening", start: "20260828T073000Z", end: "20260828T075000Z" });
  });
  ...
  it("normalizes an overnight extra (same-day 22:00 → 01:00) to end the next day", () => {
    ...
    const r = parseFavesItems({ items: [{ title: "Late social", start: "2026-08-28T22:00:00", end: "2026-08-28T01:00:00" }] });
    expect(r.ok).toBe(true);
  ...
  it("rejects zero-duration items and ends more than a day early", () => {
    const zero = parseFavesItems({ items: [{ title: "A", start: "2026-08-28T09:00:00", end: "2026-08-28T09:00:00" }] });
    expect(zero.ok).toBe(false);
    expect(zero.error).toContain("zero duration");
    const wayEarly = parseFavesItems({ items: [{ title: "A", start: "2026-08-28T09:00:00", end: "2026-08-26T09:00:00" }] });
  ...
  it("rejects a blank title and an unparseable time, naming the index", () => {
    const bad = parseFavesItems({ items: [{ title: "  ", start: "2026-08-28T13:00:00", end: "2026-08-28T14:00:00" }] });
    expect(bad.ok).toBe(false);
    expect(bad.error).toContain("items[0].title");
    const badTime = parseFavesItems({ items: [{ title: "A", start: "2026-08-28T:00", end: "2026-08-28T14:00:00" }] });
...
describe("buildFavesCalendar — the ICS document", () => {
  ...
  const two = parseFavesItems({
    items: [
      { id: "event-2", title: "Later Talk", start: "2026-08-29T20:00:00", end: "2026-08-29T21:00:00" },
      {
        id: "event-1",
        title: "Früh; und, spät",
        start: "2026-08-28T13:00:00",
  ...
  it("emits URL as a URI, never TEXT-escaping its commas/semicolons", () => {
    const withPunct = parseFavesItems({
      items: [{ id: "e", title: "A", start: "2026-08-28T13:00:00", end: "2026-08-28T14:00:00", url: "https://x.com/a,b;c" }],
    }).items;
...
const DB_DOCS = [
  ...
  { _id: "note-alice-guid-101", type: "note", userId: "alice", eventId: "guid-101", notes: "PRIVATE NOTE" },
...
describe("fetch handler — GET /faves.ics?t=<token> (subscription lane)", () => {
  ...
  it("projects requested guids from the feed, deriving a missing end from the duration", async () => {
    ...
    expect(new Date(projected[0].end).getTime()).toBe(new Date("2026-08-28T10:20:00+02:00").getTime());
  });
...
describe("fetch handler — POST /faves.ics", () => {
  ...
  it("400s malformed JSON and invalid payloads", async () => {
    ...
    const bad = await icsFetch(post({ items: [{ title: "A", start: "junk", end: "2026-08-28T14:00:00" }] }), {});
  ...
  it("returns a text/calendar attachment for a valid payload", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toBe("text/calendar; charset=utf-8");
    expect(res.headers.get("content-disposition")).toBe('attachment; filename="sotm2026-faves.ics"');
