      ...
      <div className={`max-w-6xl mx-auto ${c.cardBg} shadow-2xl ${c.border} overflow-hidden`}>
        <div className={`${c.headerBg} ${c.border} p-2.5 relative isolate`}>
            ...
            <div>
              <a href="https://2026.stateofthemap.org/" target="_blank" rel="noopener noreferrer">
                <h1 className={`text-4xl font-black ${c.bodyText} mb-[1px]`}>
                  {superMode ? "SUPER STATE OF THE MAP 2026 PICKER" : "State of the Map 2026 Picker"}
                </h1>
              </a>
          ...
          {error && error.includes("cached") && (
            <div className={`mt-0.5 ${c.accentBg} text-white px-[3px] py-0.5 rounded-lg text-sm font-bold relative z-10`}>{error}</div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-1" role="dialog" aria-modal="true">
          <div className="bg-white dark:bg-[#1b2913] rounded-2xl p-2 max-w-sm w-full shadow-2xl">
            ...
            <p className={`font-bold mb-1.5 ${c.bodyText}`}>
              This link wants to add <span className="text-[#2d6a8f]">@{friendConfirm}</span> to your crew. You'll see each other's
              schedules.
            </p>
