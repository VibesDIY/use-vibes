...
const items = (...overrides) =>
  overrides.length > 0
    ...
    : [
        {
          id: 'event-hope-cassity-fri-river',
          title: 'Hope Cassity',
...
describe('toIcsUtc — festival-local strings become ICS UTC stamps', () => {
  it('converts a naive festival-local time (AKDT, UTC-8) to Z time', () => {
    expect(toIcsUtc('2026-07-31T13:00:00')).toBe('20260731T210000Z');
  });
  it('handles DST correctly — a winter time is AKST (UTC-9)', () => {
    expect(toIcsUtc('2026-12-01T12:00:00')).toBe('20261201T210000Z');
  });
  ...
  it("accepts the feed's space-separated form", () => {
    expect(toIcsUtc('2026-07-31 13:00:00')).toBe('20260731T210000Z');
  });
...
describe('parseFavesItems — strict payload validation', () => {
  ...
  it('keeps location and http(s) url, drops a javascript: url silently', () => {
    ...
    expect(r.ok).toBe(true);
    expect(r.items[0].location).toBe('River Stage');
  ...
  it('normalizes an overnight extra (same-day 22:00 → 01:00) to end the next day', () => {
    ...
    expect(r.ok).toBe(true);
    // 22:00 AKDT Jul 30 → 06:00Z Jul 31; 01:00 AKDT bumped to Jul 31 → 09:00Z Jul 31.
    expect(r.items[0].start).toBe('20260731T060000Z');
    expect(r.items[0].end).toBe('20260731T090000Z');
  });
...
describe('buildFavesCalendar — the ICS document', () => {
  ...
  const two = parseFavesItems({
    items: [
      ...
      },
      {
        ...
        location: 'River Stage',
        url: 'https://salmonfestalaska.org/2026-lineup/',
      },
    ],
  }).items;
  ...
  it('has the calendar envelope and metadata', () => {
    ...
    expect(ics).toContain('X-WR-CALNAME:My Salmonfest Picks\r\n');
    expect(ics).toContain('X-WR-TIMEZONE:America/Anchorage\r\n');
    expect(ics.trimEnd().endsWith('END:VCALENDAR')).toBe(true);
  });
  it('emits events sorted by start time regardless of input order', () => {
    // 2026-08-01T20:00 AKDT crosses midnight UTC → 20260802T040000Z, and it must come second.
    expect(ics.indexOf('20260731T210000Z')).toBeLessThan(ics.indexOf('20260802T040000Z'));
    expect(ics.indexOf('20260802T040000Z')).toBeGreaterThan(-1);
  });
  it('stamps stable UIDs from the item id', () => {
    expect(ics).toContain('UID:event-1@salmonfest-picker.vibes.diy');
    expect(ics).toContain('UID:event-2@salmonfest-picker.vibes.diy');
  });
  it('escapes SUMMARY text and carries LOCATION/URL', () => {
    expect(ics).toContain('SUMMARY:Früh\\; und\\, spät');
    expect(ics).toContain('LOCATION:River Stage');
    expect(ics).toContain('URL:https://salmonfestalaska.org/2026-lineup/');
  });
...
describe('scheduleItemsFor — the embedded-snapshot join', () => {
  it('projects known ids into calendar items straight from the snapshot', () => {
    expect(scheduleItemsFor(['hope-cassity-fri-river'])).toEqual([
      {
        id: 'event-hope-cassity-fri-river',
        title: 'Hope Cassity',
...
const DB_DOCS = [
  {
    _id: 'favorite-Alice-hope-cassity',
    type: 'favorite',
    userId: 'Alice',
    eventId: 'hope-cassity-fri-river',
  },
  ...
  {
    _id: 'favorite-bob-goth-babe',
    type: 'favorite',
    userId: 'bob',
    eventId: 'goth-babe-fri-river',
  },
  {
    _id: 'shift-1',
    type: 'shift',
...
describe('fetch handler — GET /faves.ics?t=<token> (subscription lane)', () => {
  ...
  it('serves the never-empty anchor-only calendar before the first aggregation tick', async () => {
    ...
    expect(body).not.toContain('Hope Cassity'); // faves arrive with the tick
  });
  ...
  it("serves a user's CURRENT faves: db-aggregated ids joined against the snapshot", async () => {
    ...
    const body = await res.text();
    expect(body).toContain('SUMMARY:Hope Cassity'); // alice faved that set (case-folded handle)
    expect(body).not.toContain('Goth Babe'); // the Goth Babe set is bob's
    ...
    expect(body).toContain('LOCATION:River Stage');
    expect(body).toContain('UID:event-hope-cassity-fri-river@salmonfest-picker.vibes.diy'); // stable across refreshes
    expect(body).toContain('SUMMARY:Gates Open'); // the always-present anchor event
    expect(body).toContain('X-WR-CALNAME:@alice — Salmonfest Picks');
  ...
  it('drops a favorite the snapshot no longer lists instead of failing the feed', async () => {
    // alice also faves `act-gone`; her calendar is anchor + the Hope Cassity set + her shift.
    await tick();
  ...
  it('serves the whole feed without any egress — the snapshot replaced the fetch', async () => {
    ...
    expect(await res.text()).toContain('SUMMARY:Hope Cassity');
  });
  ...
  it('serves fave-less holders and pre-tick tokens alike: valid anchor-only, never an error', async () => {
    ...
    expect(body).toContain('X-WR-CALNAME:@jchris — Salmonfest Picks');
  ...
  it('derives legacy shift times from day + startTime/endTime, using the snapshot dates', async () => {
    ...
    expect(body).toContain('DTSTART:20260731T180000Z'); // Friday (2026-07-31) 10:00 AKDT
  });
...
describe('fetch handler — POST /faves.ics', () => {
  ...
  it('returns a text/calendar attachment for a valid payload', async () => {
    ...
    expect(res.headers.get('content-disposition')).toBe(
      'attachment; filename="salmonfest-faves.ics"'
    );
    expect(res.headers.get('cache-control')).toBe('no-store');
    const body = await res.text();
    expect(body).toContain('BEGIN:VEVENT');
    expect(body).toContain('SUMMARY:Hope Cassity');
    expect(body).toContain('DTSTART:20260731T210000Z'); // festival-local converted to UTC
  });
