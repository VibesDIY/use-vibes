...
export default function App() {
  ...
  const c = {
    ...
    header:
      'sticky top-0 z-10 bg-[#2a0a2e] text-[#fcee0a] px-4 py-3 shadow-lg border-b-4 border-[#f93c94]',
    ...
    section:
      'bg-[#2a0a2e]/90 backdrop-blur rounded-xl border-2 border-[#f93c94] p-4 shadow-[0_0_20px_rgba(249,60,148,0.4)]',
    ...
    keyWide:
      "min-w-[60px] min-h-[48px] px-2 rounded-md bg-[#f93c94] text-[#2a0a2e] font-bold font-['Orbitron',sans-serif] text-xs tracking-wider active:bg-[#fcee0a] border border-[#fcee0a]/40",
    ...
    <div className={c.page}>
      ...
      <main id="app" className={c.main}>
        <section id="how-to-play" className={c.section}>
          ...
          <p className="text-[#ffffff]/80 text-sm font-['Rajdhani',sans-serif] mb-2">
            Guess the 5-letter <span className="text-[#fcee0a] font-bold">color word</span> in 6
            tries.
          </p>
          <div className="flex gap-3 flex-wrap text-xs font-['Rajdhani',sans-serif]">
            <span className="flex items-center gap-1">
              <span className="inline-block w-7 h-7 rounded bg-[#00f0ff] text-[#2a0a2e] font-bold text-center leading-7 font-['Orbitron',sans-serif]">
                A
              </span>{' '}
              right spot
            </span>
            <span className="flex items-center gap-1">
              <span className="inline-block w-7 h-7 rounded bg-[#fcee0a] text-[#2a0a2e] font-bold text-center leading-7 font-['Orbitron',sans-serif]">
                A
              </span>{' '}
              wrong spot
            </span>
            <span className="flex items-center gap-1">
              <span className="inline-block w-7 h-7 rounded bg-[#4d1558] text-white/40 font-bold text-center leading-7 font-['Orbitron',sans-serif]">
                A
              </span>{' '}
              not in word
            </span>
          </div>
          <p className="text-[#00f0ff] text-xs mt-2 font-['Rajdhani',sans-serif]">
            Words are color names: AMBER, AZURE, CORAL, TEAL…
          </p>
        </section>
        <section id="board" className={c.section}>
          ...
          {done && (
            <div className="mt-4 text-center space-y-1">
              {doc.won ? (
                <p className="font-['Orbitron',sans-serif] text-[#00f0ff] text-xl tracking-widest">
                  🎨 YOU GOT IT!
                </p>
              ) : (
                <>
                  <p className="font-['Orbitron',sans-serif] text-[#f93c94] text-sm tracking-widest">
                    THE COLOR WAS
                  </p>
                  <p className="font-['Orbitron',sans-serif] text-[#fcee0a] text-3xl tracking-[0.3em] font-bold">
                    {answer}
                  </p>
                </>
              )}
              ...
              return (
                <div key={ri} className={c.kbRow}>
                  ...
                  {row.split('').map((k) => {
                    ...
                    const tint =
                      st === 'correct'
                        ? 'bg-[#00f0ff] text-[#2a0a2e]'
                        : st === 'present'
                          ? 'bg-[#fcee0a] text-[#2a0a2e]'
                          : st === 'absent'
                            ? 'bg-[#4d1558]/40 text-[#ffffff]/40'
                            : '';
                    return (
                      <button
                        type="button"
                        key={k}
                        className={`${c.key} ${tint} ${done ? 'opacity-60 cursor-default' : ''}`}
                        onClick={() => {
                          if (done || current.length >= 5) return;
        ...
        <section id="stats" className={c.section}>
          ...
          <div className={c.statRow}>
            <div className={c.statBox}>
              <div className={c.statNum}>{played}</div>
              <div className={c.statLabel}>Played</div>
            </div>
            <div className={c.statBox}>
              <div className={c.statNum}>{won}</div>
              <div className={c.statLabel}>Won</div>
            </div>
            <div className={c.statBox}>
              <div className={c.statNum}>{streak}</div>
              <div className={c.statLabel}>Streak</div>
            </div>
          </div>
          {done && (
            <button className={`${c.btn} mt-4`} onClick={share}>
              Share Result
            </button>
          )}
        </section>
