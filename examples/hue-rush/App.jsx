...
export default function App() {
  ...
  const c = {
    ...
    header:
      'sticky top-0 z-10 bg-[#0a2e1c] text-[#0afc6e] px-4 py-3 shadow-lg border-b-4 border-[#3c94f9]',
    ...
    section:
      'bg-[#0a2e1c]/90 backdrop-blur rounded-xl border-2 border-[#3c94f9] p-4 shadow-[0_0_20px_rgba(60,148,249,0.4)]',
    ...
    keyWide:
      "min-w-[60px] min-h-[48px] px-2 rounded-md bg-[#3c94f9] text-[#0a2e1c] font-bold font-['Orbitron',sans-serif] text-xs tracking-wider active:bg-[#0afc6e] border border-[#0afc6e]/40",
          ...
          {done && (
            <div className="mt-4 text-center space-y-2">
              {result === 'won' ? (
                ...
                <>
                  <p className="font-['Orbitron',sans-serif] text-[#3c94f9] text-sm tracking-widest">
                    THE COLOR WAS
                  </p>
                  <p className="font-['Orbitron',sans-serif] text-[#0afc6e] text-3xl tracking-[0.3em] font-bold">
                    {word}
                  </p>
                </>
              )}
              <button className={c.btn} onClick={nextRound}>
                Next Color →
              </button>
            </div>
          )}
              ...
              return (
                <div key={ri} className={c.kbRow}>
                  ...
                  {row.split('').map((k) => {
                    ...
                    const tint =
                      st === 'correct'
                        ? 'bg-[#00f0ff] text-[#0a2e1c]'
                        : st === 'present'
                          ? 'bg-[#fcee0a] text-[#0a2e1c]'
                          : st === 'absent'
                            ? 'bg-[#14503a]/40 text-[#ffffff]/40'
                            : '';
                    return (
                      <button
                        type="button"
        ...
        <section id="stats" className={c.section}>
          ...
          <div className={c.statRow}>
            <div className={c.statBox}>
              <div className={c.statNum}>{stats.played}</div>
              <div className={c.statLabel}>Rounds</div>
            </div>
            <div className={c.statBox}>
              <div className={c.statNum}>{stats.won}</div>
              <div className={c.statLabel}>Won</div>
            </div>
            <div className={c.statBox}>
              <div className={c.statNum}>{stats.streak}</div>
              <div className={c.statLabel}>Streak</div>
            </div>
            <div className={c.statBox}>
              <div className={c.statNum}>{stats.best}</div>
              <div className={c.statLabel}>Best</div>
            </div>
          </div>
        </section>
      </main>
