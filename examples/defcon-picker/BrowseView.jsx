    ...
    <div>
      <div className="mb-1.5 flex flex-wrap gap-1">
        <input
          ...
          className={`flex-1 min-w-64 p-2 m-0.5 ${c.border} rounded-2xl text-lg font-bold bg-[#1a1a20] ${c.bodyText}`}
        />
        <select
          value={selectedDay}
          onChange={(e) => setSelectedDay(e.target.value)}
          className={`p-2 m-0.5 ${c.border} rounded-2xl font-bold bg-[#1a1a20] ${c.bodyText}`}
        >
      ...
      {/* Group into per-day sections with headers, matching the My Faves page. Sessions
          carry a con-night `day` (4 AM cutoff), so a late party sorts under the
          right night. Ordered by the con day order; empty days are skipped. */}
      {(() => {
        const byDay = {};
        ...
        return daysToShow.map((day) => (
          <div key={day} className={c.schedDay}>
            <h3 className="text-xl font-black mb-1 text-[#39ff14]">
                  ...
                  >
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-1">
                      ...
                      <div className="flex gap-0.5">
                        ...
                        {event.url && (
                          <a href={event.url} target="_blank" rel="noopener noreferrer" className={c.linkBtn} title="Session link">
                            <svg
                              width="20"
                              height="20"
                              viewBox="0 0 24 24"
      ...
      {searchTerm && filteredEvents.length === 0 && (
        <div className="text-center py-3">
          <h3 className={`text-2xl font-black mb-0.5 ${c.bodyText}`}>No sessions found</h3>
          <p className={c.bodyText}>Try searching for a different session title</p>
        </div>
      )}
    </div>
  );
