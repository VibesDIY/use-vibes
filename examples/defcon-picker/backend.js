// DEF CON 34 Picker backend: proxy the schedule feed and serve faves as .ics.
//
// GET  /_api/schedule.json
//   → 200 application/json — the SCHEDULE PROXY. The upstream scheduleDays feed
//   is CORS-locked to info.defcon.org, so the client can never fetch it
//   directly; it fetches this same-origin route instead. The raw JSON text is
//   cached in module state for 10 minutes; an upstream failure serves the stale
//   copy if one exists (better a slightly old schedule than none), and 502s
//   only when there's nothing cached at all.
// POST /_api/faves.ics  { items: [{ id, title, start, end, location?, url? }] }
//   → 200 text/calendar attachment (defcon34-faves.ics) — one-shot download of
...
//   picks flow to every subscriber without re-subscribing, and session times
//   come from a join against the schedule feed (through the same 10-minute
//   cache the proxy uses — one upstream fetch serves both lanes).
//
...
// #3085). The one lane that CAN read the "defcon34" db is `scheduled` (runs
...
// Same offset trick as festival-utils.js: format the instant in the con's
// zone, re-read it as if it were UTC, and the difference is the zone offset.
// Handles DST correctly for any date (the con is PDT, but don't hardcode -7).
...
// A schedule string (naive con-local like "2026-08-07T13:00:00", or with an
// explicit offset/Z like the feed's UTC stamps) → ICS UTC basic format
// "20260807T200000Z", or null if it doesn't parse. UTC-basic strings sort
...
export const validateFavesItem = (it) => {
  ...
  // to end the next day. (+24h in absolute time; the con's dates never straddle
...
export const SCHEDULE_URL = "https://info.defcon.org/ht/defcon34/views/scheduleDays.json";

// Raw upstream JSON TEXT, cached in module state. 10 minutes matches the
...
// Same-origin schedule proxy: the client can't reach the CORS-locked upstream,
...
// Con day → calendar date, for legacy shift docs stored without absolute
...
export async function scheduled(event, ctx) {
  const docs = await ctx.db.query({ db: "defcon34" });
  const users = new Map();
...
// Project the requested session ids out of the schedule feed (through the
// shared cache) into ics-ready items. Duplicates the client flattener's rules
// on purpose (this isolate can't import festival-utils.js): id → String,
// beginIso/endIso preferred, a missing-or-equal end defaults to start + 1h,
// locationName is the location, and only the first http(s) contentEntity link
// ships (the feed mixes real URLs with fediverse handles).
export const fetchScheduleItems = async (ids) => {
  ...
  for (const day of data) {
    ...
    for (const s of day.sessions) {
      ...
      const start = String(s.beginIso ?? s.begin ?? "");
      let end = String(s.endIso ?? s.end ?? "");
...
const icsUid = (item) => {
  ...
  return `${key.replace(/[^A-Za-z0-9._-]/g, "-")}@defcon-picker.vibes.diy`;
};
...
export const buildFavesCalendar = (items, { now, calName } = {}) => {
  ...
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//vibes.diy//defcon-picker//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    `X-WR-CALNAME:${escapeIcsText(calName || "My DEF CON 34 Picks")}`,
...
const handleDownload = async (request) => {
  ...
  return new Response(ics, {
    ...
    headers: {
      ...
      "content-disposition": 'attachment; filename="defcon34-faves.ics"',
...
// empty calendar, and it's real con info.
const ANCHOR_ITEMS = [
  {
    id: "defcon34-opening",
    title: "DEF CON 34",
...
const handleSubscription = async (url) => {
  ...
  return new Response(buildFavesCalendar(items, { calName: `@${handle ?? displayName ?? "my"} — DEF CON 34 Picks` }), {
    status: 200,
    headers: {
      "content-type": "text/calendar; charset=utf-8",
...
export async function fetch(request, ctx) {
  ...
  if (url.pathname !== "/faves.ics") {
    return textResponse(404, "not found — /schedule.json (GET) or /faves.ics (POST to download, GET ?t=<token> to subscribe)");
  }
  if (request.method === "GET" || request.method === "HEAD") {
