...
export default function TracksView({
  ...
  return (
    <div>
      {/* Jump-nav: one pill per track, in the feed's own track colors. The feed adds
          tags as content lands (one-off contest tags included), so this renders
          whatever is present — never a hardcoded list that could hide a new track. */}
      ...
      {tracksList.map((track) => {
        ...
        return (
          <div key={track.title} id={trackDomId(track.title)} className="mb-2 scroll-mt-4">
            <h3
                  ...
                  <div className="space-y-[1px]">
                    {track.events.map((e) => (
                      <div key={e.eventId} className="flex items-center gap-0.5 flex-wrap">
                        ...
                        <span className={`text-sm font-bold text-[#e8e8e8]/70`}>
                        ...
                        {e.url && (
                          <a
                            ...
                            className="p-[1px] px-0.5 m-0.5 rounded-lg bg-[#1a1a20] text-[#e8e8e8] hover:bg-[#26262e] transition-all"
                            title="Session link"
                          >
                            <svg
                              width="14"
