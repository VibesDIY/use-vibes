...
export default function DefconPicker() {
  ...
  const tracksList = useMemo(() => {
    ...
    return [...map.values()].sort(
      (a, b) => b.events.length - a.events.length || a.title.localeCompare(b.title)
    );
  }, [events]);
        ...
        <div className={`${c.headerBg} ${c.border} border-t-2 border-t-[#39ff14] p-2.5`}>
          <div className="flex items-start justify-between gap-1 flex-wrap">
            <div>
              ...
              <p className={`${c.bodyText} text-base font-bold font-mono`}>
                August 5–9, 2026 · Las Vegas
              </p>
            </div>
          </div>
          {error && error.includes('cached') && (
            <div
              className={`mt-0.5 ${c.accentBg} text-[#0a0a0c] px-[3px] py-0.5 rounded-lg text-sm font-bold`}
            >
              {error}
            </div>
          )}
