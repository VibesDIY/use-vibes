...
export default function DefconPicker() {
  ...
  const { database, useLiveQuery, useDocument } = useFireproof("defcon34", {
    anonymousLocal: true,
  ...
  const getCached = () => {
    const data = localStorage.getItem("defcon34-schedule-cache");
    const ts = +localStorage.getItem("defcon34-schedule-timestamp");
    if (!data || !ts) return null;
    return { data: JSON.parse(data), isStale: Date.now() - ts > 600_000 };
  };
  const setCached = (d) => {
    localStorage.setItem("defcon34-schedule-cache", JSON.stringify(d));
    localStorage.setItem("defcon34-schedule-timestamp", Date.now().toString());
  };
  ...
  const fetchSchedule = async () => {
    ...
    try {
      // The upstream scheduleDays feed is CORS-locked to info.defcon.org, so the
      // browser can never fetch it directly — backend.js proxies it same-origin
      // (with its own 10-minute cache) at /_api/schedule.json.
  ...
  // flattenSchedule applies the feed → internal-shape rules (skip unparseable
  // begins, +1h default end, per-track tag palette, 4 AM night cutoff).
  const ingest = (data) => {
    setEvents(flattenSchedule(data));
  };

  const getDateForDay = (day) => {
    // Prefer the con day's canonical calendar date. Since a day groups
    // after-midnight sessions from the *next* calendar date (4 AM cutoff), we must
  ...
  const {
    ...
    merge: mergeShift,
    reset: resetShift,
  } = useDocument({ type: "shift", day: "Friday", startTime: "09:00", endTime: "17:00", kind: "Shift", shareWithFriends: false });
  ...
  // from the db every few minutes and re-joins session times against the
  // schedule feed on each refresh — new picks reach subscribers automatically,
  ...
  // Group sessions by their track (the tag label the feed ships in lineup.id).
  // Big tracks (Demo Labs, Workshops, …) first; the one-session contest tags sink
  // to the bottom together instead of scattering alphabetically between them.
  const tracksList = useMemo(() => {
    const map = new Map();
    for (const e of events) {
      const key = e.lineup?.id || "Event";
    ...
    return [...map.values()].sort((a, b) => b.events.length - a.events.length || a.title.localeCompare(b.title));
  }, [events]);
  ...
  const connectUrl = `https://vibes.diy/vibe/calendar/defcon-picker/?friend=${encodeURIComponent(userId)}`;
  ...
  return (
    <div className={`min-h-screen ${c.pageBg}`} style={{ touchAction: "manipulation" }}>
      <div className={`max-w-6xl mx-auto ${c.cardBg} shadow-2xl ${c.border} overflow-hidden`}>
        {/* No motif needed — the dark terminal look IS the style; a thin neon top
            border is the only ornament. */}
        <div className={`${c.headerBg} ${c.border} border-t-2 border-t-[#39ff14] p-2.5`}>
          <div className="flex items-start justify-between gap-1 flex-wrap">
            <div>
              <a href="https://defcon.org/" target="_blank" rel="noopener noreferrer">
                <h1 className="text-4xl font-black font-mono tracking-widest text-[#39ff14] mb-[1px]">
                  {superMode ? "SUPER DEF CON 34 PICKER" : "DEF CON 34 PICKER"}
                </h1>
              </a>
              <p className={`${c.bodyText} text-base font-bold font-mono`}>August 5–9, 2026 · Las Vegas</p>
            </div>
          </div>
          {error && error.includes("cached") && (
            <div className={`mt-0.5 ${c.accentBg} text-[#0a0a0c] px-[3px] py-0.5 rounded-lg text-sm font-bold`}>{error}</div>
          )}
        </div>
          ...
          {view === "schedule" && (
            <div>
              <div className="flex items-center justify-between flex-wrap gap-0.5 mb-1.5">
                <h2 className={`text-2xl font-black ${c.bodyText}`}>My DEF CON Schedule</h2>
      ...
      {friendConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-1" role="dialog" aria-modal="true">
          <div className="bg-[#141419] border border-[#39ff14]/40 rounded-2xl p-2 max-w-sm w-full shadow-2xl">
            ...
            <p className={`font-bold mb-1.5 ${c.bodyText}`}>
              This link wants to add <span className="text-[#39ff14]">@{friendConfirm}</span> to your crew. You'll see each other's
              schedules.
            </p>
