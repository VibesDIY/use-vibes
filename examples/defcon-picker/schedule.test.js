...
import { festivalDayFor, setsOnNow, upNextSets, toFestivalDate, flattenSchedule, scheduleIcsItems } from "./festival-utils.js";
    ...
    sessions: [
      {
        ...
        contentEntity: {
          title: "The Unofficial DEF CON Shoot",
          links: [
            { label: "Google Maps", type: "link", url: "https://maps.app.goo.gl/GCpWr55GjQajASA99" },
            { label: "Website", type: "link", url: "https://dcshoot.org" },
          ],
        },
        speakers: null,
        session: { timezoneName: "America/Los_Angeles" },
        tags: [{ colorBackground: "#8F6C5D", colorForeground: "#ffffff", id: 49749, label: "Event" }],
      },
    ],
  },
  {
    ...
    sessions: [
      // Shipped with end === begin — the flattener must default the end to +1 hour.
      {
        ...
        tags: [{ colorBackground: "#333dd7", colorForeground: "#ffffff", id: 49750, label: "Workshop" }],
        contentEntity: {
      ...
      { id: 68002, title: "Badge Line Hangout", beginIso: "2026-08-07T18:00:00.000Z", color: "#123456", tags: [] },
      // A party running past midnight: 01:30 PDT Saturday belongs to Friday night.
      {
        ...
        tags: [{ colorBackground: "#D56CD0", colorForeground: "#ffffff", id: 49751, label: "Party" }],
      },
...
describe("flattenSchedule — scheduleDays.json → internal event shape", () => {
  ...
  it("passes the feed's per-track palette through (bg + fg), falling back to the session color", () => {
    expect(byId["68001"].lineup).toEqual({ id: "Workshop", color: "#333dd7", textColor: "#ffffff" });
...
describe("scheduleIcsItems — flattening My Faves for the .ics backend", () => {
  ...
  it("trims whitespace-only kinds and titles instead of shipping strings the backend rejects", () => {
    const items = scheduleIcsItems({
      events: [
        ...
        { eventId: "2", title: "  Real Talk  ", start: "2026-08-07T13:00:00", end: "2026-08-07T14:00:00" },
      ],
      shifts: [{ _id: "s1", kind: "   ", start: "2026-08-06T09:00:00", end: "2026-08-06T17:00:00" }],
      shiftStart,
      shiftEnd,
    });
    expect(items.map((i) => [i.id, i.title])).toEqual([
  ...
  it("drops zero-duration shifts but keeps overnight ones for the backend to normalize", () => {
    const items = scheduleIcsItems({
      ...
      shifts: [
        ...
        { _id: "overnight", kind: "Line con", start: "2026-08-06T22:00:00", end: "2026-08-06T01:00:00" },
      ],
      shiftStart,
      shiftEnd,
    });
  ...
  it("drops entries the backend would reject: malformed times and blank titles", () => {
    const items = scheduleIcsItems({
      events: [{ eventId: "1", title: "", start: "2026-08-07T13:00:00", end: "2026-08-07T14:00:00" }],
      shifts: [
